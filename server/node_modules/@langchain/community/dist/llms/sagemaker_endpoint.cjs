...
let _aws_sdk_client_sagemaker_runtime = require("@aws-sdk/client-sagemaker-runtime");
//#region src/llms/sagemaker_endpoint.ts
var sagemaker_endpoint_exports = /* @__PURE__ */ require_runtime.__exportAll({
...
* endpoint.
*/
var SageMakerEndpoint = class extends _langchain_core_language_models_llms.LLM {
	...
	constructor(fields) {
		...
		if (!fields?.endpointName) throw new Error(`Please pass an "endpointName" field to the constructor`);
		if (!fields?.contentHandler) throw new Error(`Please pass a "contentHandler" field to the constructor`);
		...
		this.client = new _aws_sdk_client_sagemaker_runtime.SageMakerRuntimeClient(fields.clientOptions);
	}
	_llmType() {
	...
	*/
	...
	async noStreamingCall(prompt, options) {
		...
		const response = await this.caller.call(() => this.client.send(new _aws_sdk_client_sagemaker_runtime.InvokeEndpointCommand({
	...
	*/
	async *_streamResponseChunks(prompt, options, runManager) {
		const body = await this.contentHandler.transformInput(prompt, this.modelKwargs ?? {});
		const { contentType, accepts } = this.contentHandler;
		const stream = await this.caller.call(() => this.client.send(new _aws_sdk_client_sagemaker_runtime.InvokeEndpointWithResponseStreamCommand({
			EndpointName: this.endpointName,
			Body: body,
			ContentType: contentType,
			Accept: accepts,
