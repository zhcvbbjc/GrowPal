...
let _layerup_layerup_security = require("@layerup/layerup-security");
//#region src/llms/layerup_security.ts
var layerup_security_exports = /* @__PURE__ */ require_runtime.__exportAll({ LayerupSecurity: () => LayerupSecurity });
...
var LayerupSecurity = class extends _langchain_core_language_models_llms.LLM {
	static lc_name() {
	...
	constructor(options) {
		...
		this.layerup = new _layerup_layerup_security.LayerupSecurity({
	...
	async _call(input, options) {
		...
		if (this.promptGuardrails.length > 0) {
			...
			if (!securityResponse.all_safe) return this.handlePromptGuardrailViolation(securityResponse).content;
		}
		...
		if (this.responseGuardrails.length > 0) {
			...
			if (!securityResponse.all_safe) return this.handleResponseGuardrailViolation(securityResponse).content;
		}
		return result;
	}
};
