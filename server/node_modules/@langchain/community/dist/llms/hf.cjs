...
var hf_exports = /* @__PURE__ */ require_runtime.__exportAll({ HuggingFaceInference: () => HuggingFaceInference });
/**
...
* ```
*/
var HuggingFaceInference = class HuggingFaceInference extends _langchain_core_language_models_llms.LLM {
	...
	async *_streamResponseChunks(prompt, options, runManager) {
		...
		for await (const chunk of stream) {
			...
			if (chunk.generated_text) yield new _langchain_core_outputs.GenerationChunk({
				text: "",
	...
	async _call(prompt, options) {
		...
		return (await this.caller.callWithOptions({ signal: options.signal }, hfi.textGeneration.bind(hfi), args)).generated_text;
	}
	/** @ignore */
