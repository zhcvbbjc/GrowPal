...
let _raycast_api = require("@raycast/api");
//#region src/llms/raycast.ts
var raycast_exports = /* @__PURE__ */ require_runtime.__exportAll({ RaycastAI: () => RaycastAI });
...
*/
var RaycastAI = class extends _langchain_core_language_models_llms.LLM {
	...
	*/
	constructor(fields) {
		super(fields ?? {});
		if (!_raycast_api.environment.canAccess(_raycast_api.AI)) throw new Error("Raycast AI environment is not accessible.");
	...
	*/
	async _call(prompt, options) {
		return await this.caller.call(async () => {
			...
			return await _raycast_api.AI.ask(prompt, {
				model: this.model,
