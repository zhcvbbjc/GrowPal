...
var replicate_exports = /* @__PURE__ */ require_runtime.__exportAll({ Replicate: () => Replicate });
/**
...
* ```
*/
var Replicate = class Replicate extends _langchain_core_language_models_llms.LLM {
	static lc_name() {
		return "Replicate";
	}
	get lc_secrets() {
	...
	constructor(fields) {
		super(fields);
		const apiKey = fields?.apiKey ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("REPLICATE_API_KEY") ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("REPLICATE_API_TOKEN");
	...
	async *_streamResponseChunks(prompt, options, runManager) {
		...
		for await (const chunk of stream) {
			...
			if (chunk.event === "done") yield new _langchain_core_outputs.GenerationChunk({
				text: "",
	...
	async _prepareReplicate() {
		return new (await (Replicate.imports())).Replicate({
			userAgent: "langchain",
			auth: this.apiKey
		});
	}
	async _getReplicateInput(replicate, prompt) {
		if (this.promptKey === void 0) {
			...
			const inputProperties = (await replicate.models.versions.get(modelString.split("/")[0], modelString.split("/")[1], versionString)).openapi_schema?.components?.schemas?.Input?.properties;
			if (inputProperties === void 0) this.promptKey = "prompt";
			else this.promptKey = Object.entries(inputProperties).sort(([_keyA, valueA], [_keyB, valueB]) => {
				return (valueA["x-order"] || 0) - (valueB["x-order"] || 0);
			})[0][0] ?? "prompt";
		}
		return {
