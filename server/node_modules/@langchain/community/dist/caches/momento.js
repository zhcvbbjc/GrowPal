...
var momento_exports = /* @__PURE__ */ __exportAll({ MomentoCache: () => MomentoCache });
/**
...
* ```
*/
var MomentoCache = class MomentoCache extends BaseCache {
	client;
	cacheName;
	ttlSeconds;
	constructor(props) {
	...
	*/
	async lookup(prompt, llmKey) {
		...
		} else if (getResponse instanceof CacheGet.Miss) return null;
		else if (getResponse instanceof CacheGet.Error) throw getResponse.innerException();
	...
	*/
	async update(prompt, llmKey, value) {
		...
		if (setResponse instanceof CacheSet.Success) {} else if (setResponse instanceof CacheSet.Error) throw setResponse.innerException();
		else throw new Error(`Unknown response type: ${setResponse.toString()}`);
	}
};
//#endregion
export { MomentoCache, momento_exports };
