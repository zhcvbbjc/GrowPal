...
var ibm_exports = /* @__PURE__ */ require_runtime.__exportAll({ WatsonxRerank: () => WatsonxRerank });
var WatsonxRerank = class extends _langchain_core_retrievers_document_compressors.BaseDocumentCompressor {
	maxRetries = 0;
	version = "2024-05-31";
	truncateInputTokens;
	returnOptions;
	...
	async compressDocuments(documents, query) {
		...
		return result.results.map(({ index, score }) => {
	...
	async rerank(documents, query, options) {
		...
		const { result } = await new _langchain_core_utils_async_caller.AsyncCaller({
			maxConcurrency: this.maxConcurrency,
			maxRetries: this.maxRetries
		}).call(() => this.service.textRerank({
			...this.scopeId(),
			inputs,
			query,
