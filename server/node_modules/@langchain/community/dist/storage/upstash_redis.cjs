...
var upstash_redis_exports = /* @__PURE__ */ require_runtime.__exportAll({ UpstashRedisStore: () => UpstashRedisStore });
/**
...
* ```
*/
var UpstashRedisStore = class extends _langchain_core_stores.BaseStore {
	...
	constructor(fields) {
		...
		else if (fields.config) this.client = new _upstash_redis.Redis(fields.config);
	...
	*/
	async mget(keys) {
		...
		return (await this.client.mget(...prefixedKeys)).map((value) => {
			if (!value) return;
