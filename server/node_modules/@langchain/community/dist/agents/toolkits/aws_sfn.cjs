...
let _langchain_classic_agents = require("@langchain/classic/agents");
...
* ```
*/
var AWSSfnToolkit = class extends _langchain_core_tools.Toolkit {
	tools;
	stateMachineArn;
	asl;
	constructor(args) {
...
function createAWSSfnAgent(llm, toolkit, args) {
	...
	const agent = new _langchain_classic_agents.ZeroShotAgent({
		llmChain: new _langchain_classic_chains.LLMChain({
			prompt: _langchain_classic_agents.ZeroShotAgent.createPrompt(tools, {
	...
	return _langchain_classic_agents.AgentExecutor.fromAgentAndTools({
		agent,
		tools,
