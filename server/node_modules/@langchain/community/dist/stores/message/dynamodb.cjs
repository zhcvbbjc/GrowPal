...
var dynamodb_exports = /* @__PURE__ */ require_runtime.__exportAll({ DynamoDBChatMessageHistory: () => DynamoDBChatMessageHistory });
/**
...
* class.
*/
var DynamoDBChatMessageHistory = class extends _langchain_core_chat_history.BaseListChatMessageHistory {
	lc_namespace = [
		"langchain",
		"stores",
		"message",
	...
	*/
	...
	constructor({ tableName, sessionId, partitionKey, sortKey, messageAttributeName, config, key = {} }) {
		...
		this.client = new _aws_sdk_client_dynamodb.DynamoDBClient(config ?? {});
	...
	*/
	async getMessages() {
		try {
			...
			const response = await this.client.send(new _aws_sdk_client_dynamodb.GetItemCommand(params));
			return (0, _langchain_core_messages.mapStoredMessagesToChatMessages)((response.Item ? response.Item[this.messageAttributeName]?.L ?? [] : []).filter((item) => item.M !== void 0).map((item) => {
				const data = {
					role: item.M?.role?.S,
	...
	*/
	async addMessage(message) {
		try {
			...
			await this.client.send(new _aws_sdk_client_dynamodb.UpdateItemCommand(params));
		} catch (error) {
	...
	*/
	async addMessages(messages) {
		try {
			const dynamoMessages = (0, _langchain_core_messages.mapChatMessagesToStoredMessages)(messages).map(this.createDynamoDBSerializedChatMessage);
			const params = {
				TableName: this.tableName,
				Key: this.dynamoKey,
				ExpressionAttributeNames: { "#m": this.messageAttributeName },
	...
	*/
	async clear() {
		try {
			...
			await this.client.send(new _aws_sdk_client_dynamodb.DeleteItemCommand(params));
		} catch (error) {
