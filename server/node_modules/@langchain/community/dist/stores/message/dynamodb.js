...
var dynamodb_exports = /* @__PURE__ */ __exportAll({ DynamoDBChatMessageHistory: () => DynamoDBChatMessageHistory });
/**
...
var DynamoDBChatMessageHistory = class extends BaseListChatMessageHistory {
	...
	*/
	async getMessages() {
		try {
			...
			return mapStoredMessagesToChatMessages((response.Item ? response.Item[this.messageAttributeName]?.L ?? [] : []).filter((item) => item.M !== void 0).map((item) => {
				const data = {
					role: item.M?.role?.S,
	...
	*/
	async addMessages(messages) {
		try {
			const dynamoMessages = mapChatMessagesToStoredMessages(messages).map(this.createDynamoDBSerializedChatMessage);
			const params = {
				TableName: this.tableName,
				Key: this.dynamoKey,
				ExpressionAttributeNames: { "#m": this.messageAttributeName },
