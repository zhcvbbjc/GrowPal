...
let _xata_io_client = require("@xata.io/client");
//#region src/stores/message/xata.ts
var xata_exports = /* @__PURE__ */ require_runtime.__exportAll({ XataChatMessageHistory: () => XataChatMessageHistory });
...
* ```
*/
var XataChatMessageHistory = class extends _langchain_core_chat_history.BaseListChatMessageHistory {
	lc_namespace = [
		"langchain",
		"stores",
		"message",
	...
	constructor(fields) {
		...
		else if (config) this.client = new _xata_io_client.BaseClient(config);
		...
		if (fields.createTable !== false) {
			...
			this.apiClient = new _xata_io_client.XataApiClient({ apiKey });
	...
	*/
	async getMessages() {
		...
		return (0, _langchain_core_messages.mapStoredMessagesToChatMessages)((await this.client.db[this.table].filter({ sessionId: this.sessionId }).sort("xata.createdAt", "asc").getAll()).map((message) => {
			const data = {
	...
	*/
	async clear() {
		await this.ensureTable();
		const ids = (await this.client.db[this.table].select(["id"]).filter({ sessionId: this.sessionId }).getAll()).map((m) => m.id);
	...
	*/
	async ensureTable() {
		...
		const urlParts = (0, _xata_io_client.parseWorkspacesUrlParts)(host);
