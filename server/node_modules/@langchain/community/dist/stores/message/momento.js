...
import { CacheDelete, CacheListFetch, CacheListPushBack, CollectionTtl, InvalidArgumentError } from "@gomomento/sdk-core";
import { mapChatMessagesToStoredMessages, mapStoredMessagesToChatMessages } from "@langchain/core/messages";
import { BaseListChatMessageHistory } from "@langchain/core/chat_history";
//#region src/stores/message/momento.ts
var momento_exports = /* @__PURE__ */ __exportAll({ MomentoChatMessageHistory: () => MomentoChatMessageHistory });
/**
...
* ```
*/
var MomentoChatMessageHistory = class MomentoChatMessageHistory extends BaseListChatMessageHistory {
	lc_namespace = [
		"langchain",
		"stores",
		"message",
	...
	constructor(props) {
		...
		this.sessionTtl = props.sessionTtl !== void 0 ? CollectionTtl.of(props.sessionTtl) : CollectionTtl.fromCacheTtl();
	}
	/**
	...
	*/
	static async fromProps(props) {
		...
		if (props.ensureCacheExists || props.ensureCacheExists === void 0) await ensureCacheExists(props.client, props.cacheName);
		return instance;
	}
	/**
	...
	*/
	validateTtlSeconds(ttlSeconds) {
		if (ttlSeconds !== void 0 && ttlSeconds <= 0) throw new InvalidArgumentError("ttlSeconds must be positive.");
	}
	/**
	...
	*/
	async getMessages() {
		...
		let messages = [];
		if (fetchResponse instanceof CacheListFetch.Hit) messages = fetchResponse.valueList().map((serializedStoredMessage) => JSON.parse(serializedStoredMessage));
		else if (fetchResponse instanceof CacheListFetch.Miss) {} else if (fetchResponse instanceof CacheListFetch.Error) throw fetchResponse.innerException();
	...
	*/
	async addMessage(message) {
		...
		if (pushResponse instanceof CacheListPushBack.Success) {} else if (pushResponse instanceof CacheListPushBack.Error) throw pushResponse.innerException();
	...
	*/
	async clear() {
		...
		if (deleteResponse instanceof CacheDelete.Success) {} else if (deleteResponse instanceof CacheDelete.Error) throw deleteResponse.innerException();
		else throw new Error(`Unknown response type: ${deleteResponse.toString()}`);
	}
};
//#endregion
export { MomentoChatMessageHistory, momento_exports };
