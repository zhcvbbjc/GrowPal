...
var file_system_exports = /* @__PURE__ */ require_runtime.__exportAll({
...
* ```
*/
var FileSystemChatMessageHistory = class extends _langchain_core_chat_history.BaseListChatMessageHistory {
	lc_namespace = [
		"langchain",
		"stores",
		"message",
	...
	constructor(chatHistoryInput) {
		...
		this.filePath = chatHistoryInput.filePath ?? ".history/history.json";
	}
	async init() {
		if (store) return;
		try {
	...
	async loadStore() {
		try {
			const store = await node_fs.promises.readFile(this.filePath, "utf-8");
	...
	async getMessages() {
		await this.init();
		return (0, _langchain_core_messages.mapStoredMessagesToChatMessages)(store[this.userId]?.[this.sessionId]?.messages ?? []);
	}
	async addMessage(message) {
		await this.init();
		const messages = await this.getMessages();
		messages.push(message);
		const storedMessages = (0, _langchain_core_messages.mapChatMessagesToStoredMessages)(messages);
		store[this.userId] ??= {};
	...
	async getAllSessions() {
		await this.init();
		return store[this.userId] ? Object.entries(store[this.userId]).map(([id, session]) => ({
			id,
