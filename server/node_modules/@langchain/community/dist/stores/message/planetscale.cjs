...
var planetscale_exports = /* @__PURE__ */ require_runtime.__exportAll({ PlanetScaleChatMessageHistory: () => PlanetScaleChatMessageHistory });
/**
...
* ```
*/
var PlanetScaleChatMessageHistory = class extends _langchain_core_chat_history.BaseListChatMessageHistory {
	lc_namespace = [
		"langchain",
		"stores",
		"message",
	...
	constructor(fields) {
		...
		else if (config) this.client = new _planetscale_database.Client(config);
	...
	*/
	async getMessages() {
		...
		return (0, _langchain_core_messages.mapStoredMessagesToChatMessages)((await this.connection.execute(query, params)).rows.map((message) => {
			const data = {
