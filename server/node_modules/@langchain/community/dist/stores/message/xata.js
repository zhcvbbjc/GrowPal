...
var xata_exports = /* @__PURE__ */ __exportAll({ XataChatMessageHistory: () => XataChatMessageHistory });
...
var XataChatMessageHistory = class extends BaseListChatMessageHistory {
	...
	*/
	async getMessages() {
		...
		return mapStoredMessagesToChatMessages((await this.client.db[this.table].filter({ sessionId: this.sessionId }).sort("xata.createdAt", "asc").getAll()).map((message) => {
			const data = {
