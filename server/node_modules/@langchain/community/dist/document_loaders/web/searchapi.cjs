...
var searchapi_exports = /* @__PURE__ */ require_runtime.__exportAll({ SearchApiLoader: () => SearchApiLoader });
/**
...
* ```
*/
var SearchApiLoader = class extends _langchain_core_document_loaders_base.BaseDocumentLoader {
	apiKey;
	parameters;
	constructor(params) {
		super();
		const { apiKey = (0, _langchain_core_utils_env.getEnvironmentVariable)("SEARCHAPI_API_KEY") } = params;
	...
	*/
	processResponseData(data) {
		...
		for (const responseType of [
			...
			"organic_results",
			"transcripts"
		]) if (responseType in data) documents.push(...this.extractDocuments(data[responseType], responseType));
		return documents;
	}
	/**
