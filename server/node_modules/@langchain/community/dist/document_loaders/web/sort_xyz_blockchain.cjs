...
var sort_xyz_blockchain_exports = /* @__PURE__ */ require_runtime.__exportAll({ SortXYZBlockchainLoader: () => SortXYZBlockchainLoader });
/**
...
* ```
*/
var SortXYZBlockchainLoader = class extends _langchain_core_document_loaders_base.BaseDocumentLoader {
	contractAddress;
	blockchain;
	apiKey;
	queryType;
	...
	*/
	async load() {
		...
		while (true) {
			...
			try {
				const fullResponse = await (await fetch("https://api.sort.xyz/v1/queries/run", {
					method: "POST",
					headers: {
