...
var confluence_exports = /* @__PURE__ */ require_runtime.__exportAll({ ConfluencePagesLoader: () => ConfluencePagesLoader });
/**
...
* ```
*/
var ConfluencePagesLoader = class extends _langchain_core_document_loaders_base.BaseDocumentLoader {
	baseUrl;
	spaceKey;
	username;
	accessToken;
	...
	*/
	get authorizationHeader() {
		...
		else if (this.username && this.accessToken) return `Basic ${Buffer.from(`${this.username}:${this.accessToken}`).toString("base64")}`;
	}
	/**
	...
	*/
	async load(options) {
		try {
			return (await this.fetchAllPagesInSpace(options?.start, options?.limit)).map((page) => this.createDocumentFromPage(page));
		} catch (error) {
	...
	*/
	createDocumentFromPage(page) {
		const htmlWithoutOtherMacros = page.body.storage.value.replace(/<ac:structured-macro\s+ac:name="(attachments|view-file)"[^>]*(?:\/?>|>.*?<\/ac:structured-macro>)/gs, "[ATTACHMENT]");
		const codeBlocks = [];
		let plainTextContent = (0, html_to_text.htmlToText)(htmlWithoutOtherMacros.replace(/<ac:structured-macro.*?<ac:parameter ac:name="language">(.*?)<\/ac:parameter>.*?<ac:plain-text-body><!\[CDATA\[([\s\S]*?)\]\]><\/ac:plain-text-body><\/ac:structured-macro>/g, (_, language, code) => {
		...
		return new _langchain_core_documents.Document({
			pageContent: plainTextContent.replace(/^\s*[\r\n]/gm, ""),
			metadata: {
				id: page.id,
				status: page.status,
				title: page.title,
