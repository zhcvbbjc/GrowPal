...
var sitemap_exports = /* @__PURE__ */ __exportAll({ SitemapLoader: () => SitemapLoader });
...
var SitemapLoader = class extends CheerioWebBaseLoader {
	...
	async _loadSitemapUrls(elements) {
		return (await CheerioWebBaseLoader.scrapeAll(elements.map((ele) => ele.loc), this.caller, this.timeout, this.textDecoder)).map(($, i) => {
	...
	async load() {
		const chunks = chunkArray(await this.parseSitemap(), this.chunkSize);
		const documents = [];
