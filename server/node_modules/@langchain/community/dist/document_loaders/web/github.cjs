...
ignore = require_runtime.__toESM(ignore);
let binary_extensions = require("binary-extensions");
binary_extensions = require_runtime.__toESM(binary_extensions);
//#region src/document_loaders/web/github.ts
var github_exports = /* @__PURE__ */ require_runtime.__exportAll({ GithubRepoLoader: () => GithubRepoLoader });
...
*/
var GithubRepoLoader = class GithubRepoLoader extends _langchain_core_document_loaders_base.BaseDocumentLoader {
	baseUrl;
	apiUrl;
	owner;
	repo;
	...
	ignorePaths;
	submoduleInfos;
	constructor(githubUrl, { accessToken = (0, _langchain_core_utils_env.getEnvironmentVariable)("GITHUB_ACCESS_TOKEN"), baseUrl = "https://github.com", apiUrl = "https://api.github.com", branch = "main", recursive = true, processSubmodules = false, unknown = _langchain_classic_document_loaders_fs_directory.UnknownHandling.Warn, ignoreFiles = [], ignorePaths, verbose = false, maxConcurrency = 2, maxRetries = 2, ...rest } = {}) {
		super();
	...
	*/
	async load() {
		...
		const documents = (await this.processRepo()).map((fileResponse) => new _langchain_core_documents.Document({
	...
	*/
	async getSubmoduleInfo() {
		...
		const gitmodulesFile = (await this.fetchRepoFiles("")).filter(({ name }) => name === ".gitmodules")?.[0];
		if (gitmodulesFile) {
	...
	*/
	async *loadSubmoduleAsStream(submoduleInfo) {
		...
		this.log(`Accessing submodule ${submoduleInfo.name} (${submoduleInfo.url})...`);
		yield* await new GithubRepoLoader(submoduleInfo.url, {
			accessToken: this.accessToken,
			baseUrl: this.baseUrl,
			apiUrl: this.apiUrl,
			branch: submoduleInfo.ref,
			...
			maxConcurrency: this.maxConcurrency,
			maxRetries: this.maxRetries
		}).processRepoAsStream(submoduleInfo.path);
	}
	/**
	...
	*/
	async fetchFileContentWrapper(file) {
		return {
			contents: await this.fetchFileContent(file).catch((error) => {
	...
	*/
	async fetchFileContent(file) {
		return this.caller.call(async () => {
			...
			return (await fetch(file.download_url, { headers: this.headers })).text();
		});
	}
	/**
	...
	*/
	handleError(message) {
		switch (this.unknown) {
			case _langchain_classic_document_loaders_fs_directory.UnknownHandling.Ignore: break;
			case _langchain_classic_document_loaders_fs_directory.UnknownHandling.Warn:
				console.warn(message);
				break;
			case _langchain_classic_document_loaders_fs_directory.UnknownHandling.Error: throw new Error(message);
