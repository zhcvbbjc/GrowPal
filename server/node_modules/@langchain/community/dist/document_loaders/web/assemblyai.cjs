...
let assemblyai = require("assemblyai");
//#region src/document_loaders/web/assemblyai.ts
var assemblyai_exports = /* @__PURE__ */ require_runtime.__exportAll({
...
*/
var AssemblyAILoader = class extends _langchain_core_document_loaders_base.BaseDocumentLoader {
	client;
	/**
	...
	*/
	constructor(assemblyAIOptions) {
		...
		if (!options) options = {};
		if (!options.apiKey) options.apiKey = (0, _langchain_core_utils_env.getEnvironmentVariable)("ASSEMBLYAI_API_KEY");
...
var AudioTranscriptParagraphsLoader = class extends CreateTranscriptLoader {
	...
	*/
	async load() {
		const transcript = await this.transcribeOrGetTranscript();
		return (await this.client.transcripts.paragraphs(transcript.id)).paragraphs.map((p) => new _langchain_core_documents.Document({
			pageContent: p.text,
			metadata: p
		}));
	}
...
var AudioTranscriptSentencesLoader = class extends CreateTranscriptLoader {
	...
	*/
	async load() {
		const transcript = await this.transcribeOrGetTranscript();
		return (await this.client.transcripts.sentences(transcript.id)).sentences.map((p) => new _langchain_core_documents.Document({
			pageContent: p.text,
			metadata: p
		}));
	}
...
var AudioSubtitleLoader = class extends CreateTranscriptLoader {
	...
	*/
	async load() {
		const transcript = await this.transcribeOrGetTranscript();
		return [new _langchain_core_documents.Document({ pageContent: await this.client.transcripts.subtitles(transcript.id, this.subtitleFormat) })];
	}
};
//#endregion
