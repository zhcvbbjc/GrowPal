...
let _notionhq_client = require("@notionhq/client");
let notion_to_md = require("notion-to-md");
let notion_to_md_build_utils_notion_js = require("notion-to-md/build/utils/notion.js");
//#region src/document_loaders/web/notionapi.ts
var notionapi_exports = /* @__PURE__ */ require_runtime.__exportAll({
...
const isPageResponse = (res) => !(0, _notionhq_client.isNotionClientError)(res) && res.object === "page";
const isDatabaseResponse = (res) => !(0, _notionhq_client.isNotionClientError)(res) && res.object === "database";
const isErrorResponse = (res) => (0, _notionhq_client.isNotionClientError)(res);
const isPage = (res) => isPageResponse(res) && (0, _notionhq_client.isFullPage)(res);
const isDatabase = (res) => isDatabaseResponse(res) && (0, _notionhq_client.isFullDatabase)(res);
/**
...
* ```
*/
var NotionAPILoader = class extends _langchain_core_document_loaders_base.BaseDocumentLoader {
	caller;
	notionClient;
	n2mClient;
	id;
	...
	constructor(options) {
		super();
		this.caller = new _langchain_core_utils_async_caller.AsyncCaller({
			maxConcurrency: 64,
			...options.callerOptions
		});
		this.notionClient = new _notionhq_client.Client({
			logger: () => {},
	...
	*/
	async loadBlock(block) {
		...
		if (block.has_children) {
			...
			mdBlock.children = await this.loadBlocks(await this.caller.call(() => (0, notion_to_md_build_utils_notion_js.getBlockChildren)(this.notionClient, block_id, null)));
		}
		return mdBlock;
	}
	/**
	...
	*/
	async loadBlocks(blocksResponse) {
		const blocks = blocksResponse.filter(_notionhq_client.isFullBlock);
	...
	*/
	async loadPage(page) {
		...
		if (!(0, _notionhq_client.isFullPage)(pageDetails)) {
			this.pageCompleted.push(pageId);
			return;
		}
		const mdBlocks = await this.loadBlocks(pageBlocks);
		let pageContent = this.n2mClient.toMarkdownString(mdBlocks).parent;
		...
		const pageDocument = new _langchain_core_documents.Document({
			pageContent,
			metadata
		});
	...
	*/
	async loadDatabase(id) {
		try {
			for await (const page of (0, _notionhq_client.iteratePaginatedAPI)(this.notionClient.databases.query, {
				database_id: id,
				page_size: 50
	...
	*/
	async load() {
		...
		if (errors.length === 2) {
			if (errors.every((e) => e.code === _notionhq_client.APIErrorCode.ObjectNotFound)) throw new AggregateError([Error(`Could not find object with ID: ${this.id}. Make sure the relevant pages and databases are shared with your integration.`), ...errors]);
