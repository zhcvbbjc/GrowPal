...
var google_cloud_storage_exports = /* @__PURE__ */ __exportAll({ GoogleCloudStorageLoader: () => GoogleCloudStorageLoader });
/**
...
var GoogleCloudStorageLoader = class extends BaseDocumentLoader {
	...
	async load() {
		...
		try {
			const [buffer] = await new Storage(this.storageOptions).bucket(this.bucket).file(this.file).download();
		...
		try {
			return await new UnstructuredLoader(filePath, this.unstructuredLoaderOptions).load();
		} catch {
