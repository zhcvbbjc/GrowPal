...
let youtubei_js = require("youtubei.js");
//#region src/document_loaders/web/youtube.ts
var youtube_exports = /* @__PURE__ */ require_runtime.__exportAll({ YoutubeLoader: () => YoutubeLoader });
/**
...
* ```
*/
var YoutubeLoader = class YoutubeLoader extends _langchain_core_document_loaders_base.BaseDocumentLoader {
	videoId;
	language;
	addVideoInfo;
	constructor(config) {
	...
	*/
	async load() {
		...
		try {
			const info = await (await youtubei_js.Innertube.create({
				lang: this.language,
				retrieve_player: false
			})).getInfo(this.videoId);
			transcript = (await info.getTranscript()).transcript.content?.body?.initial_segments.map((segment) => segment.snippet.text).join(" ") ?? "";
