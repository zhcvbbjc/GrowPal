...
var assemblyai_exports = /* @__PURE__ */ __exportAll({
...
*/
var AssemblyAILoader = class extends BaseDocumentLoader {
	client;
	/**
	...
	*/
	constructor(assemblyAIOptions) {
		...
		if (!options) options = {};
		if (!options.apiKey) options.apiKey = getEnvironmentVariable("ASSEMBLYAI_API_KEY");
...
var AudioTranscriptParagraphsLoader = class extends CreateTranscriptLoader {
	...
	*/
	async load() {
		const transcript = await this.transcribeOrGetTranscript();
		return (await this.client.transcripts.paragraphs(transcript.id)).paragraphs.map((p) => new Document({
			pageContent: p.text,
			metadata: p
		}));
	}
...
var AudioTranscriptSentencesLoader = class extends CreateTranscriptLoader {
	...
	*/
	async load() {
		const transcript = await this.transcribeOrGetTranscript();
		return (await this.client.transcripts.sentences(transcript.id)).sentences.map((p) => new Document({
			pageContent: p.text,
			metadata: p
		}));
	}
...
var AudioSubtitleLoader = class extends CreateTranscriptLoader {
	...
	*/
	async load() {
		const transcript = await this.transcribeOrGetTranscript();
		return [new Document({ pageContent: await this.client.transcripts.subtitles(transcript.id, this.subtitleFormat) })];
	}
};
//#endregion
export { AudioSubtitleLoader, AudioTranscriptLoader, AudioTranscriptParagraphsLoader, AudioTranscriptSentencesLoader, assemblyai_exports };
