...
let _getzep_zep_js = require("@getzep/zep-js");
//#region src/memory/zep.ts
var zep_exports = /* @__PURE__ */ require_runtime.__exportAll({
	ZepMemory: () => ZepMemory,
...
const condenseZepMemoryIntoHumanMessage = (memory) => {
	...
	return new _langchain_core_messages.HumanMessage(`${systemPrompt ? `${systemPrompt}\n` : ""}${concatMessages}`);
};
/**
...
const zepMemoryToMessages = (memory, humanPrefix = "Human", aiPrefix = "AI") => {
	...
	if (memory && memory.messages) messages = messages.concat(memory.messages.filter((m) => m.content).map((message) => {
		...
		if (role === humanPrefix) return new _langchain_core_messages.HumanMessage(content);
		else if (role === aiPrefix) return new _langchain_core_messages.AIMessage(content);
		else return new _langchain_core_messages.ChatMessage(content, role);
	}));
	return messages;
};
/**
...
var ZepMemory = class extends require_memory_chat_memory.chat_memory_exports.BaseChatMemory {
	...
	*/
	...
	constructor(fields) {
		...
		this.zepClientPromise = _getzep_zep_js.ZepClient.init(this.baseURL, fields.apiKey);
	}
	get memoryKeys() {
	...
	*/
	async loadMemoryVariables(values) {
		...
		try {
			...
			if (error instanceof _getzep_zep_js.NotFoundError) return this.returnMessages ? { [this.memoryKey]: [] } : { [this.memoryKey]: "" };
			else throw error;
		}
		const memoryData = {
		...
		return { [this.memoryKey]: this.separateMessages ? (0, _langchain_core_messages.getBufferString)(zepMemoryToMessages(memoryData, this.humanPrefix, this.aiPrefix), this.humanPrefix, this.aiPrefix) : condenseZepMemoryIntoHumanMessage(memoryData).content };
	}
	/**
	...
	*/
	async saveContext(inputValues, outputValues) {
		...
		const memory = new _getzep_zep_js.Memory({ messages: [new _getzep_zep_js.Message({
			role: this.humanPrefix,
			content: `${input}`
		}), new _getzep_zep_js.Message({
			role: this.aiPrefix,
			content: `${output}`
		})] });
		const zepClient = await this.zepClientPromise;
