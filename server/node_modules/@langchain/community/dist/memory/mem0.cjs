...
const condenseMem0MemoryIntoHumanMessage = (memory) => {
	return new _langchain_core_messages.HumanMessage(`These are the memories I have stored. Give more weightage to the question by users and try to answer that first. You have to modify your answer based on the memories I have provided. If the memories are irrelevant you can ignore them. Also don't reply to this section of the prompt, or the memories, they are only for your reference. The MEMORIES of the USER are: 

\n${mem0MemoryContextToSystemPrompt(memory)}`);
};
/**
...
const mem0MemoryToMessages = (memories) => {
	...
	if (memoryContent) messages.push(new _langchain_core_messages.SystemMessage(memoryContent));
	memories.forEach((memory) => {
		if (memory.messages) memory.messages.forEach((msg) => {
			const content = typeof msg.content === "string" ? msg.content : JSON.stringify(msg.content);
			if (msg.role === "user") messages.push(new _langchain_core_messages.HumanMessage(content));
			else if (msg.role === "assistant") messages.push(new _langchain_core_messages.AIMessage(content));
			else if (content) messages.push(new _langchain_core_messages.ChatMessage(content, msg.role));
		});
	});
	return messages;
};
...
var Mem0Memory = class extends require_memory_chat_memory.chat_memory_exports.BaseChatMemory {
	...
	*/
	async loadMemoryVariables(values) {
		...
		return { [this.memoryKey]: this.separateMessages ? (0, _langchain_core_messages.getBufferString)(mem0MemoryToMessages(memories), this.humanPrefix, this.aiPrefix) : condenseMem0MemoryIntoHumanMessage(memories).content ?? "" };
	}
	/**
