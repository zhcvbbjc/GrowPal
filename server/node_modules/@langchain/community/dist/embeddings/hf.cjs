...
let _huggingface_inference = require("@huggingface/inference");
//#region src/embeddings/hf.ts
var hf_exports = /* @__PURE__ */ require_runtime.__exportAll({ HuggingFaceInferenceEmbeddings: () => HuggingFaceInferenceEmbeddings });
/**
* Class that extends the Embeddings class and provides methods for
* generating embeddings using Hugging Face models through the
* HuggingFaceInference API.
*/
var HuggingFaceInferenceEmbeddings = class extends _langchain_core_embeddings.Embeddings {
	apiKey;
	model;
	endpointUrl;
	provider;
	...
	constructor(fields) {
		...
		this.apiKey = fields?.apiKey ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("HUGGINGFACEHUB_API_KEY");
		this.endpointUrl = fields?.endpointUrl;
		this.provider = fields?.provider;
		this.client = this.endpointUrl ? new _huggingface_inference.InferenceClient(this.apiKey).endpoint(this.endpointUrl) : new _huggingface_inference.InferenceClient(this.apiKey);
	}
	async _embed(texts) {
