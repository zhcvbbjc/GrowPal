...
var ibm_exports = /* @__PURE__ */ require_runtime.__exportAll({ WatsonxEmbeddings: () => WatsonxEmbeddings });
var WatsonxEmbeddings = class extends _langchain_core_embeddings.Embeddings {
	model;
	serviceUrl;
	version;
	spaceId;
	...
	async listModels() {
		if (this.service) {
			...
			return (await new _langchain_core_utils_async_caller.AsyncCaller({
				maxConcurrency: this.maxConcurrency,
				maxRetries: this.maxRetries
			}).call(() => service.listFoundationModelSpecs(listModelParams))).result.resources?.map((item) => item.model_id);
	...
	async embedSingleText(inputs) {
		...
		if ("modelId" in scopeId && this.service) {
			...
			return (await new _langchain_core_utils_async_caller.AsyncCaller({
				maxConcurrency: this.maxConcurrency,
				maxRetries: this.maxRetries
			}).call(() => service.embedText({
				inputs,
				...scopeId,
				parameters: this.invocationParams()
			}))).result.results.map((item) => item.embedding);
		} else if (this.gateway && "model" in scopeId) {
			const { gateway } = this;
			return (await new _langchain_core_utils_async_caller.AsyncCaller({
				maxConcurrency: this.maxConcurrency,
				maxRetries: this.maxRetries
			}).call(() => gateway.embeddings.completion.create({
				input: inputs,
				...scopeId
	...
	async embedDocuments(documents) {
		return await this.embedSingleText(documents);
	}
	async embedQuery(document) {
		return (await this.embedSingleText([document]))[0];
	}
};
//#endregion
