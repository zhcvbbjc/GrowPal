...
var deepinfra_exports = /* @__PURE__ */ require_runtime.__exportAll({ DeepInfraEmbeddings: () => DeepInfraEmbeddings });
/**
...
* ```
*/
var DeepInfraEmbeddings = class extends _langchain_core_embeddings.Embeddings {
	apiToken;
	batchSize;
	modelName;
	/**
	...
	*/
	constructor(fields) {
		...
		const apiKey = fieldsWithDefaults?.apiToken || (0, _langchain_core_utils_env.getEnvironmentVariable)(API_TOKEN_ENV_VAR);
	...
	*/
	async embedDocuments(inputs) {
		const batches = (0, _langchain_core_utils_chunk_array.chunkArray)(inputs, this.batchSize);
	...
	*/
	async embeddingWithRetry(request) {
		return await this.caller.call(() => fetch(`https://api.deepinfra.com/v1/inference/${this.modelName}`, {
			method: "POST",
			headers: {
