...
* ```
*/
var ChatTencentHunyuan = class extends _langchain_core_language_models_chat_models.BaseChatModel {
	static lc_name() {
	...
	constructor(fields) {
		super(fields ?? {});
		this.tencentSecretId = fields?.tencentSecretId ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("TENCENT_SECRET_ID");
		if (!this.tencentSecretId) throw new Error("Tencent SecretID not found");
		this.tencentSecretKey = fields?.tencentSecretKey ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("TENCENT_SECRET_KEY");
	...
	async *_streamResponseChunks(messages, options, runManager) {
		...
		for await (const chunk of stream) {
			...
			yield new _langchain_core_outputs.ChatGenerationChunk({
				text: Content,
				message: new _langchain_core_messages.AIMessageChunk({ content: Content }),
