...
const require_anthropic = require("../../utils/bedrock/anthropic.cjs");
...
let _smithy_signature_v4 = require("@smithy/signature-v4");
let _smithy_protocol_http = require("@smithy/protocol-http");
let _smithy_eventstream_codec = require("@smithy/eventstream-codec");
let _smithy_util_utf8 = require("@smithy/util-utf8");
let _aws_crypto_sha256_js = require("@aws-crypto/sha256-js");
...
var web_exports = /* @__PURE__ */ require_runtime.__exportAll({
...
function convertOneMessageToText(message, humanPrompt, aiPrompt) {
	...
	else if (_langchain_core_messages.ChatMessage.isInstance(message)) return `\n\n${message.role[0].toUpperCase() + message.role.slice(1)}: {message.content}`;
	throw new Error(`Unknown role: ${message._getType()}`);
}
function convertMessagesToPromptAnthropic(messages, humanPrompt = "\n\nHuman:", aiPrompt = "\n\nAssistant:") {
	const messagesCopy = [...messages];
	if (messagesCopy.length === 0 || messagesCopy[messagesCopy.length - 1]._getType() !== "ai") messagesCopy.push(new _langchain_core_messages.AIMessage({ content: "" }));
...
function formatTools(tools) {
	...
	if (tools.every(_langchain_core_utils_function_calling.isLangChainTool)) return tools.map((tc) => ({
		name: tc.name,
		description: tc.description,
		input_schema: (0, _langchain_core_utils_types.isInteropZodSchema)(tc.schema) ? (0, _langchain_core_utils_json_schema.toJsonSchema)(tc.schema) : tc.schema
	}));
	if (tools.every(_langchain_core_language_models_base.isOpenAITool)) return tools.map((tc) => ({
		name: tc.function.name,
		description: tc.function.description,
		input_schema: tc.function.parameters
	}));
	if (tools.every(require_anthropic.isAnthropicTool)) return tools;
	if (tools.some(_langchain_core_utils_function_calling.isStructuredTool) || tools.some(_langchain_core_language_models_base.isOpenAITool) || tools.some(require_anthropic.isAnthropicTool)) throw new Error("All tools passed to BedrockChat must be of the same type.");
...
* </details>
*/
var BedrockChat = class extends _langchain_core_language_models_chat_models.BaseChatModel {
	...
	endpointHost;
	modelKwargs;
	codec = new _smithy_eventstream_codec.EventStreamCodec(_smithy_util_utf8.toUtf8, _smithy_util_utf8.fromUtf8);
	streaming = false;
	...
	constructor(fields) {
		const awsAccessKeyId = fields?.awsAccessKeyId ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("AWS_ACCESS_KEY_ID");
		const awsSecretAccessKey = fields?.awsSecretAccessKey ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("AWS_SECRET_ACCESS_KEY");
		const awsSessionToken = fields?.awsSessionToken ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("AWS_SESSION_TOKEN");
	...
	async _generateNonStreaming(messages, options, _runManager) {
		const endpointHost = this.endpointHost ?? `bedrock-runtime.${this.region}.amazonaws.com`;
		...
		if (this.usesMessagesApi) {
			const outputGeneration = require_index.BedrockLLMInputOutputAdapter.prepareMessagesOutput(provider, json);
	...
	async _signedFetch(messages, options, fields) {
		...
		const inputBody = this.usesMessagesApi ? require_index.BedrockLLMInputOutputAdapter.prepareMessagesInput(provider, messages, max_tokens, temperature, stop, modelKwargs, guardrailConfig, tools) : require_index.BedrockLLMInputOutputAdapter.prepareInput(provider, convertMessagesToPromptAnthropic(messages), max_tokens, temperature, stop, modelKwargs, fields.bedrockMethod, guardrailConfig);
		const url = new URL(`https://${endpointHost}/model/${this.model}/${bedrockMethod}`);
		const request = new _smithy_protocol_http.HttpRequest({
		...
		const signedRequest = await new _smithy_signature_v4.SignatureV4({
			credentials: this.credentials,
			service: "bedrock",
			region: this.region,
			sha256: _aws_crypto_sha256_js.Sha256
		}).sign(request);
		return await this.caller.callWithOptions({ signal: options.signal }, async () => this.fetchFn(url, {
	...
	async *_streamResponseChunks(messages, options, runManager) {
		...
		if (provider === "anthropic" || provider === "cohere" || provider === "meta" || provider === "mistral") {
			const toolsInParams = require_anthropic._toolsInParams(options);
			...
			for await (const chunk of this._readChunks(reader)) {
				...
				if (body.bytes !== void 0) {
					...
					if (this.usesMessagesApi) {
						const chunk = require_index.BedrockLLMInputOutputAdapter.prepareMessagesOutput(provider, chunkResult, { coerceContentToString: !toolsInParams });
						if (chunk === void 0) continue;
						if (provider === "anthropic" && chunk.generationInfo?.usage !== void 0) delete chunk.generationInfo.usage;
						const finalMetrics = chunk.generationInfo?.["amazon-bedrock-invocationMetrics"];
						if (finalMetrics != null && typeof finalMetrics === "object" && (0, _langchain_core_messages.isAIMessage)(chunk.message)) chunk.message.usage_metadata = {
						...
						if (isChatGenerationChunk(chunk)) {
							yield chunk;
							runManager?.handleLLMNewToken(chunk.text, void 0, void 0, void 0, void 0, { chunk });
						} else runManager?.handleLLMNewToken(chunk.text);
					} else {
						...
						const chunk = new _langchain_core_outputs.ChatGenerationChunk({
							text,
							message: new _langchain_core_messages.AIMessageChunk({ content: text })
						});
						yield chunk;
						runManager?.handleLLMNewToken(text, void 0, void 0, void 0, void 0, { chunk });
					}
				}
			}
		} else {
	...
	_readChunks(reader) {
		...
		function getMessageLength(buffer) {
			...
			return new DataView(buffer.buffer, buffer.byteOffset, buffer.byteLength).getUint32(0, false);
		}
	...
	bindTools(tools, _kwargs) {
		if (this.modelProvider !== "anthropic") throw new Error("Currently, tool calling through Bedrock is only supported for Anthropic models.");
