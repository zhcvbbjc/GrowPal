...
var friendli_exports = /* @__PURE__ */ require_runtime.__exportAll({ ChatFriendli: () => ChatFriendli });
...
function friendliResponseToChatMessage(message) {
	switch (message.role) {
		case "user": return new _langchain_core_messages.HumanMessage(message.content ?? "");
		case "assistant": return new _langchain_core_messages.AIMessage(message.content ?? "");
		case "system": return new _langchain_core_messages.SystemMessage(message.content ?? "");
...
function _convertDeltaToMessageChunk(delta) {
	...
	else if (role === "system") return new _langchain_core_messages.SystemMessageChunk({ content });
...
*/
var ChatFriendli = class extends _langchain_core_language_models_chat_models.BaseChatModel {
