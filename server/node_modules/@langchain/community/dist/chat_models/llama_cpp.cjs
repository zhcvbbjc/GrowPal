...
var llama_cpp_exports = /* @__PURE__ */ require_runtime.__exportAll({ ChatLlamaCpp: () => ChatLlamaCpp });
/**
...
* ```
*/
var ChatLlamaCpp = class ChatLlamaCpp extends _langchain_core_language_models_chat_models.SimpleChatModel {
	static inputs;
	maxTokens;
	temperature;
	topK;
	...
	*/
	static async initialize(inputs) {
		...
		instance._model = await require_llama_cpp.createLlamaModel(inputs, await (0, node_llama_cpp.getLlama)());
	...
	async *_streamResponseChunks(input, _options, runManager) {
		...
		for await (const chunk of stream) {
			yield new _langchain_core_outputs.ChatGenerationChunk({
				...
				message: new _langchain_core_messages.AIMessageChunk({ content: this._model.detokenize([chunk]) }),
				generationInfo: {}
			});
	...
	_buildSession(messages) {
		...
		if (sysMessage !== "" && interactions.length > 0) {
			this._session = new node_llama_cpp.LlamaChatSession({
				contextSequence: this._context.getSequence(),
				systemPrompt: sysMessage
			});
			this._session.setChatHistory(interactions);
		} else if (sysMessage !== "" && interactions.length === 0) this._session = new node_llama_cpp.LlamaChatSession({
			contextSequence: this._context.getSequence(),
			systemPrompt: sysMessage
		});
		else if (sysMessage === "" && interactions.length > 0) {
			this._session = new node_llama_cpp.LlamaChatSession({ contextSequence: this._context.getSequence() });
			this._session.setChatHistory(interactions);
		} else this._session = new node_llama_cpp.LlamaChatSession({ contextSequence: this._context.getSequence() });
		return prompt;
	}
	...
	_buildPrompt(input) {
		return input.map((message) => {
			...
			else if (_langchain_core_messages.ChatMessage.isInstance(message)) messageText = `\n\n${message.role[0].toUpperCase()}${message.role.slice(1)}: ${message.content}`;
			else {
