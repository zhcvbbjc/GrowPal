...
var minimax_exports = /* @__PURE__ */ __exportAll({ ChatMinimax: () => ChatMinimax });
/**
...
* ```
*/
var ChatMinimax = class extends BaseChatModel {
	static lc_name() {
		return "ChatMinimax";
	}
	get callKeys() {
	...
	*/
	invocationParams(options) {
		return {
			...
			functions: options?.functions ?? (options?.tools ? options?.tools.map(convertToOpenAIFunction) : void 0),
	...
	*/
	messageToMinimaxMessage(messages, options) {
		return messages?.filter((message) => {
			if (ChatMessage.isInstance(message)) return message.role !== "system";
...
function minimaxResponseToChatMessage(message) {
	switch (message.sender_type) {
		case "USER": return new HumanMessage(message.text || "");
		case "BOT": return new AIMessage(message.text || "", { function_call: message.function_call });
		case "FUNCTION": return new AIMessage(message.text || "");
		default: return new ChatMessage(message.text || "", message.sender_type ?? "unknown");
	}
}
//#endregion
export { ChatMinimax, minimax_exports };
