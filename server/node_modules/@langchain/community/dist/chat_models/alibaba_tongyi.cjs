...
var alibaba_tongyi_exports = /* @__PURE__ */ require_runtime.__exportAll({ ChatAlibabaTongyi: () => ChatAlibabaTongyi });
/**
...
function parseRawToolCalls(rawToolCalls, partial = false) {
	...
	for (const rawToolCall of rawToolCalls) {
		const normalizedToolCall = normalizeToolCall(rawToolCall);
		try {
			const parsedToolCall = (0, _langchain_core_output_parsers_openai_tools.parseToolCall)(normalizedToolCall, {
				returnId: true,
				partial
			});
			...
			invalidToolCalls.push((0, _langchain_core_output_parsers_openai_tools.makeInvalidToolCall)(normalizedToolCall, errorMessage));
		}
	}
	return {
		toolCalls,
...
* ```
*/
var ChatAlibabaTongyi = class extends _langchain_core_language_models_chat_models.BaseChatModel {
	static lc_name() {
	...
	*/
	...
	constructor(fields = {}) {
		super(fields);
		this.alibabaApiKey = fields?.alibabaApiKey ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("ALIBABA_API_KEY");
		if (!this.alibabaApiKey) throw new Error("Ali API key not found");
		this.region = fields.region ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("ALIBABA_REGION") ?? "china";
	...
	*/
	invocationParams(options) {
		const tools = options?.tools?.map((tool) => (0, _langchain_core_utils_function_calling.convertToOpenAITool)(tool)) ?? [];
	...
	*/
	...
	withStructuredOutput(outputSchema, config) {
		...
		const description = (0, _langchain_core_utils_types.getSchemaDescription)(schema) ?? "A function available to call.";
		const method = config?.method;
		const includeRaw = config?.includeRaw;
		if (method === "jsonMode") throw new Error(`ChatAlibabaTongyi only supports "functionCalling" for structured output.`);
		let functionName = name ?? "extract";
		const outputFormatSchema = (0, _langchain_core_utils_types.isInteropZodSchema)(schema) ? (0, _langchain_core_utils_json_schema.toJsonSchema)(schema) : schema;
		let tools;
		if ((0, _langchain_core_utils_types.isInteropZodSchema)(schema)) tools = [{
			type: "function",
			function: {
				name: functionName,
				description,
		...
		const outputParser = _langchain_core_runnables.RunnableLambda.from((input) => {
		...
		const parserAssign = _langchain_core_runnables.RunnablePassthrough.assign({ parsed: (input, cfg) => outputParser.invoke(input.raw, cfg) });
	...
	async _generate(messages, options, runManager) {
		...
		return {
			generations: [{
				text,
				message: rawToolCalls.length > 0 ? new _langchain_core_messages.AIMessage({
					content: text,
				...
				}) : new _langchain_core_messages.AIMessage({
					content: text,
					usage_metadata: usageMetadata,
					response_metadata: {
						model_provider: "alibaba_tongyi",
	...
	async *createTongyiStream(request, signal) {
		...
		const stream = _langchain_core_utils_stream.IterableReadableStream.fromReadableStream(response.body);
