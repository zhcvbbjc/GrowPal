...
var googlevertexai_exports = /* @__PURE__ */ require_runtime.__exportAll({ GoogleVertexAIMultimodalEmbeddings: () => GoogleVertexAIMultimodalEmbeddings });
/**
...
*/
var GoogleVertexAIMultimodalEmbeddings = class extends _langchain_core_embeddings.Embeddings {
	...
	*/
	async embedMediaQuery(media) {
		const instances = [this.mediaToInstance(media)];
		const responses = await this.connection.request(instances, {}, {});
		return this.responseToEmbeddings(responses)[0];
	}
	/**
