...
var datadog_exports = /* @__PURE__ */ require_runtime.__exportAll({ DatadogLLMObsTracer: () => DatadogLLMObsTracer });
var DatadogLLMObsTracer = class extends _langchain_core_tracers_base.BaseTracer {
	name = "datadog_tracer";
	ddLLMObsEndpoint;
	endpoint = (0, _langchain_core_utils_env.getEnvironmentVariable)("DD_LLMOBS_ENDPOINT") || "https://api.datadoghq.com/api/unstable/llm-obs/v1/trace/spans";
	...
	constructor(fields) {
		...
		const apiKey = ddApiKey || (0, _langchain_core_utils_env.getEnvironmentVariable)("DD_API_KEY");
	...
	uuidToBigInt(uuid) {
		const first64Bits = uuid.replace(/-/g, "").slice(0, 16);
		return BigInt(`0x${first64Bits}`).toString();
	}
	...
	transformOutput(outputs, spanKind) {
		...
		if (spanKind === "llm") return {
			output: { messages: outputs?.generations?.flatMap((generations) => generations.map(({ message, text }) => {
				if ((0, _langchain_core_messages.isAIMessage)(message) && message?.usage_metadata) {
	...
	langchainRunToDatadogLLMObsSpan(run) {
		...
		const durationNs = Number(this.milisecondsToNanoseconds(run.end_time)) - startTimeNs;
