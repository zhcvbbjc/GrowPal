...
var llm_exports = /* @__PURE__ */ require_runtime.__exportAll({
...
`;
const DEFAULT_PROMPT = /* @__PURE__ */ _langchain_core_prompts.ChatPromptTemplate.fromMessages([["system", SYSTEM_PROMPT], ["human", "Tip: Make sure to answer in the correct format and do not include any explanations. Use the given format to extract information from the following input: {input}"]]);
...
function createOptionalEnumType({ enumValues = void 0, description = "", isRel = false }) {
	...
	if (enumValues && enumValues.length) schema = zod_v3.z.enum(enumValues).describe(`${description} Available options are: ${enumValues.join(", ")}.`);
		...
		const additionalInfo = isRel ? "Instead of using specific and momentary types such as 'BECAME_PROFESSOR', use more general and timeless relationship types like 'PROFESSOR'. However, do not sacrifice any accuracy for generality" : "Ensure you use basic or elementary types for node labels.\nFor example, when you identify an entity representing a person, always label it as **'Person'**. Avoid using more specific terms like 'Mathematician' or 'Scientist'";
...
var LLMGraphTransformer = class {
	...
	constructor({ llm, allowedNodes = [], allowedRelationships = [], prompt = DEFAULT_PROMPT, strictMode = true, nodeProperties = [], relationshipProperties = [], fallbackRelationshipType = null }) {
		...
		const structuredLLM = llm.withStructuredOutput((0, _langchain_core_utils_json_schema.toJsonSchema)(schema));
	...
	*/
	async processResponse(document) {
		...
		if (this.strictMode && (this.allowedNodes.length > 0 || this.allowedRelationships.length > 0)) {
			...
			if (this.allowedRelationships.length > 0) relationships = relationships.filter((rel) => this.allowedRelationships.map((rel) => rel.toLowerCase()).includes(rel.type.toLowerCase()));
		}
