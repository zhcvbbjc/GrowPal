...
var analyticdb_exports = /* @__PURE__ */ __exportAll({ AnalyticDBVectorStore: () => AnalyticDBVectorStore });
		...
		try {
			...
			for (let i = 0; i < documents.length; i += 1) {
				...
				if (chunksTableData.length === chunkSize) {
					...
					await pipeline(rs, client.query(from(`COPY ${this.collectionName}(id, embedding, document, metadata) FROM STDIN`)));
