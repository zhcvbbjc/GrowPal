...
var hanavector_exports = /* @__PURE__ */ require_runtime.__exportAll({ HanaDB: () => HanaDB });
...
var HanaDB = class HanaDB extends _langchain_core_vectorstores.VectorStore {
	connection;
	distanceStrategy;
	...
	*/
	static parseFloatArrayFromString(arrayAsString) {
		return arrayAsString.slice(1, -1).split(",").map((x) => parseFloat(x));
	}
	/**
	...
	*/
	async checkColumn(tableName, columnName, columnType, columnLength) {
      ...
      AND COLUMN_NAME = ?`;
		...
		if (resultSet.length === 0) throw new Error(`Column ${columnName} does not exist`);
			...
			if (!(Array.isArray(columnType) ? columnType.includes(dataType) : columnType === dataType)) throw new Error(`Column ${columnName} has the wrong type: ${dataType}`);
	...
	async tableExists(tableName) {
		...
		if ((await this.executeStatement(statement, [tableName]))[0].COUNT === 1) return true;
		return false;
	}
	/**
			...
			if (typeof filterValue === "number") if (Number.isInteger(filterValue)) queryTuple.push(filterValue.toString());
				...
				if (specialOp in COMPARISONS_TO_SQL) {
					...
					if (Array.isArray(specialVal)) {
						sqlParam = `(${Array(specialVal.length).fill("?").join(",")})`;
	...
	*/
	async similaritySearchVectorWithScore(queryEmbedding, k, filter) {
		return (await this.similaritySearchWithScoreAndVectorByVector(queryEmbedding, k, filter)).map(([doc, score]) => [doc, score]);
	}
	/**
	...
	*/
	async similaritySearchWithScoreAndVectorByVector(embedding, k, filter) {
      ...
      FROM ${HanaDB.escapeSqlIdentifier(this.tableName)}`;
		...
		const client = this.connection;
		const statement = await this.prepareQuery(client, query);
		return (await this.executeStatement(statement, queryTuple)).map((row) => {
	...
	*/
	async maxMarginalRelevanceSearch(query, options) {
		...
		return (0, _langchain_core_utils_math.maximalMarginalRelevance)(queryEmbedding, docs.map((doc) => doc[2]), lambda, k).map((index) => docs[index][0]);
	}
};
//#endregion
