...
var supabase_exports = /* @__PURE__ */ require_runtime.__exportAll({ SupabaseVectorStore: () => SupabaseVectorStore });
/**
...
* <br />
*/
var SupabaseVectorStore = class SupabaseVectorStore extends _langchain_core_vectorstores.VectorStore {
	client;
	tableName;
	queryName;
	filter;
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
		return (await this._searchSupabase(query, k, filter)).map((resp) => [new _langchain_core_documents.Document({
	...
	*/
	async maxMarginalRelevanceSearch(query, options) {
		...
		return (0, _langchain_core_utils_math.maximalMarginalRelevance)(queryEmbedding, searches.map((searchResp) => searchResp.embedding), options.lambda, options.k).map((idx) => new _langchain_core_documents.Document({
