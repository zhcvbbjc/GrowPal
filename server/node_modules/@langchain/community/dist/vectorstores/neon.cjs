...
var neon_exports = /* @__PURE__ */ require_runtime.__exportAll({ NeonPostgres: () => NeonPostgres });
/**
...
*/
var NeonPostgres = class NeonPostgres extends _langchain_core_vectorstores.VectorStore {
	tableName;
	schemaName;
	idColumnName;
	vectorColumnName;
	...
	constructor(embeddings, config) {
		...
		this._verbose = config.verbose ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("LANGCHAIN_VERBOSE") === "true";
	...
	*/
    ...
    `;
		...
		return await (0, _neondatabase_serverless.neon)(this.neonConnectionString).query(queryString, flatValues);
	}
	/**
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
      ...
      LIMIT $2;`;
		const documents = await (0, _neondatabase_serverless.neon)(this.neonConnectionString).query(queryString, parameters);
		const results = [];
	...
	*/
	async delete(params) {
		const sql = (0, _neondatabase_serverless.neon)(this.neonConnectionString);
