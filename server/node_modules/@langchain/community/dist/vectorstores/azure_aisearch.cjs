...
let _azure_search_documents = require("@azure/search-documents");
//#region src/vectorstores/azure_aisearch.ts
var azure_aisearch_exports = /* @__PURE__ */ require_runtime.__exportAll({
...
*/
var AzureAISearchVectorStore = class AzureAISearchVectorStore extends _langchain_core_vectorstores.VectorStore {
	get lc_secrets() {
		return {
	...
	constructor(embeddings, config) {
		...
		const endpoint = config.endpoint ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("AZURE_AISEARCH_ENDPOINT");
		const key = config.key ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("AZURE_AISEARCH_KEY");
		...
		if (!config.client) {
			credentials ??= new _azure_search_documents.AzureKeyCredential(key);
			this.client = new _azure_search_documents.SearchClient(endpoint, this.indexName, credentials, { userAgentOptions: { userAgentPrefix: USER_AGENT_PREFIX } });
			const indexClient = new _azure_search_documents.SearchIndexClient(endpoint, credentials, { userAgentOptions: { userAgentPrefix: USER_AGENT_PREFIX } });
	...
	*/
	async deleteMany(filter) {
		...
		const deleteResults = [];
		const bufferedClient = new _azure_search_documents.SearchIndexingBufferedSender(this.client, (entity) => entity.id);
	...
	*/
	async deleteById(ids) {
		const docs = (Array.isArray(ids) ? ids : [ids]).map((id) => ({ id }));
		const deleteResults = [];
		const bufferedClient = new _azure_search_documents.SearchIndexingBufferedSender(this.client, (entity) => entity.id);
		bufferedClient.on("batchSucceeded", (response) => {
			deleteResults.push(...response.results);
	...
	*/
	async similaritySearch(query, k = 4, filter = void 0) {
		return (await this.similaritySearchWithScore(query, k, filter)).map((result) => result[0]);
	}
	/**
	* Performs a similarity search using query type specified in configuration.
	...
	*/
	async maxMarginalRelevanceSearch(query, options) {
		...
		return (0, _langchain_core_utils_math.maximalMarginalRelevance)(queryEmbedding, docs.map((doc) => doc[0].metadata.embedding), lambda, k).map((index) => {
	...
	*/
	async createSearchIndexDefinition(indexName) {
		const embeddingDimensions = (await this.embeddings.embedQuery("test")).length;
		return {
			name: indexName,
			vectorSearch: {
				algorithms: [{
...
//#endregion
exports.AzureAISearchQueryType = AzureAISearchQueryType;
exports.AzureAISearchVectorStore = AzureAISearchVectorStore;
Object.defineProperty(exports, "azure_aisearch_exports", {
	enumerable: true,
	get: function() {
		return azure_aisearch_exports;
	}
});

//# sourceMappingURL=azure_aisearch.cjs.map