...
*/
var CloseVector = class extends _langchain_core_vectorstores.SaveableVectorStore {
	_instance;
	credentials;
	_vectorstoreType() {
		return "closevector";
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
		return (await this.instance.similaritySearchVectorWithScore(query, k, filter ? (x) => filter?.({
			pageContent: x.pageContent,
			metadata: x.metadata || {}
		}) || false : void 0)).map((x) => [new _langchain_core_documents.Document({
