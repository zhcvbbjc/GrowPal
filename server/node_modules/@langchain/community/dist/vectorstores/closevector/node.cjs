...
let closevector_node = require("closevector-node");
//#region src/vectorstores/closevector/node.ts
var node_exports = /* @__PURE__ */ require_runtime.__exportAll({ CloseVectorNode: () => CloseVectorNode });
/**
...
var CloseVectorNode = class CloseVectorNode extends require_common.CloseVector {
	...
	*/
	static async loadFromCloud(options) {
		...
		return new this(options.embeddings, instance.args, options.credentials);
	}
	/**
	...
	*/
	static async load(directory, embeddings, credentials) {
		...
		return new this(embeddings, instance.args, credentials);
	}
	/**
	...
	*/
	static async fromDocuments(docs, embeddings, args, credentials) {
		const instance = new this(embeddings, args || { space: "cosine" }, credentials);
