...
var couchbase_search_exports = /* @__PURE__ */ require_runtime.__exportAll({ CouchbaseSearchVectorStore: () => CouchbaseSearchVectorStore });
/**
...
*/
var CouchbaseSearchVectorStore = class CouchbaseSearchVectorStore extends _langchain_core_vectorstores.VectorStore {
	...
	*/
	async checkIndexExists() {
		if (this.scopedIndex) {
			if (!(await this._scope.searchIndexes().getAllIndexes()).map((index) => index.name).includes(this.indexName)) throw new Error(`Index ${this.indexName} does not exist. Please create the index before searching.`);
		} else if (!(await this.cluster.searchIndexes().getAllIndexes()).map((index) => index.name).includes(this.indexName)) throw new Error(`Index ${this.indexName} does not exist. Please create the index before searching.`);
		return true;
	}
	/**
