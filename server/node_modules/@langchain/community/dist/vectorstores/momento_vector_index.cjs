...
var momento_vector_index_exports = /* @__PURE__ */ require_runtime.__exportAll({ MomentoVectorIndex: () => MomentoVectorIndex });
/**
...
*/
var MomentoVectorIndex = class MomentoVectorIndex extends _langchain_core_vectorstores.VectorStore {
	client;
	indexName;
	textField;
	_ensureIndexExists;
	...
	*/
	async ensureIndexExists(numDimensions) {
		...
		if (response instanceof _gomomento_sdk_core.CreateVectorIndex.Success) return true;
		else if (response instanceof _gomomento_sdk_core.CreateVectorIndex.AlreadyExists) return false;
		else if (response instanceof _gomomento_sdk_core.CreateVectorIndex.Error) throw new Error(response.toString());
	...
	*/
	async addVectors(vectors, documents, documentProps) {
		...
		for (let i = 0; i < numBatches; i += 1) {
			...
			if (response instanceof _gomomento_sdk_core.VectorUpsertItemBatch.Success) continue;
			else if (response instanceof _gomomento_sdk_core.VectorUpsertItemBatch.Error) throw new Error(response.toString());
			else throw new Error(`Unknown response type: ${response.toString()}`);
		}
	}
	/**
	...
	*/
	async delete(params) {
		...
		if (response instanceof _gomomento_sdk_core.VectorDeleteItemBatch.Success) {} else if (response instanceof _gomomento_sdk_core.VectorDeleteItemBatch.Error) throw new Error(response.toString());
		else throw new Error(`Unknown response type: ${response.toString()}`);
	}
	/**
	...
	*/
	async similaritySearchVectorWithScore(query, k) {
		const response = await this.client.search(this.indexName, query, {
			topK: k,
			metadataFields: _gomomento_sdk_core.ALL_VECTOR_METADATA
		});
		if (response instanceof _gomomento_sdk_core.VectorSearch.Success) {
			if (response.hits === void 0) return [];
			return response.hits().map((hit) => [new _langchain_core_documents.Document({
		...
		} else if (response instanceof _gomomento_sdk_core.VectorSearch.Error) throw new Error(response.toString());
		else throw new Error(`Unknown response type: ${response.toString()}`);
	}
	/**
	...
	*/
	async maxMarginalRelevanceSearch(query, options) {
		...
		if (response instanceof _gomomento_sdk_core.VectorSearchAndFetchVectors.Success) {
			const hits = response.hits();
			return (0, _langchain_core_utils_math.maximalMarginalRelevance)(queryEmbedding, hits.map((hit) => hit.vector), options.lambda, options.k).map((index) => {
				const { [this.textField]: pageContent, ...metadata } = hits[index].metadata;
				return new _langchain_core_documents.Document({
					metadata,
					pageContent
				});
			});
		} else if (response instanceof _gomomento_sdk_core.VectorSearchAndFetchVectors.Error) throw new Error(response.toString());
		else throw new Error(`Unknown response type: ${response.toString()}`);
	}
	/**
...
Object.defineProperty(exports, "momento_vector_index_exports", {
	enumerable: true,
	get: function() {
		return momento_vector_index_exports;
	}
});

//# sourceMappingURL=momento_vector_index.cjs.map