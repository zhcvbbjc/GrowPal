...
var turbopuffer_exports = /* @__PURE__ */ require_runtime.__exportAll({ TurbopufferVectorStore: () => TurbopufferVectorStore });
/**
...
*/
var TurbopufferVectorStore = class extends _langchain_core_vectorstores.VectorStore {
	get lc_secrets() {
	...
	constructor(embeddings, args) {
		...
		const apiKey = argsApiKey ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("TURBOPUFFER_API_KEY");
	...
	async addVectors(vectors, documents, options) {
		...
		const batchedVectors = (0, _langchain_core_utils_chunk_array.chunkArray)(vectors, this.batchSize);
		const batchedDocuments = (0, _langchain_core_utils_chunk_array.chunkArray)(documents, this.batchSize);
		const batchedIds = options?.ids ? (0, _langchain_core_utils_chunk_array.chunkArray)(options.ids, this.batchSize) : batchedDocuments.map((docs) => docs.map((_) => (0, uuid.v4)()));
	...
	async similaritySearchVectorWithScore(query, k, filter) {
		return (await this.queryVectors(query, k, false, filter)).map((res) => {
