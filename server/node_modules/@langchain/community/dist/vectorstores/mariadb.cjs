...
mariadb = require_runtime.__toESM(mariadb);
//#region src/vectorstores/mariadb.ts
var mariadb_exports = /* @__PURE__ */ require_runtime.__exportAll({ MariaDBStore: () => MariaDBStore });
...
* <br />
*/
var MariaDBStore = class MariaDBStore extends _langchain_core_vectorstores.VectorStore {
	tableName;
	collectionTableName;
	...
	*/
	escapeId(identifier, alwaysQuote) {
		...
		if (/^[0-9a-zA-Z$_]*$/.test(identifier)) {
	...
	async loadCollectionId() {
		...
		if (this.collectionTableName) {
			...
			if (queryResult.length > 0) this.collectionId = queryResult[0][0];
				...
				this.collectionId = (await this.pool.query({
					sql: insertString,
					rowsAsArray: true
				}, [this.collectionName, this.collectionMetadata]))[0][0];
			}
		}
	}
	/**
	...
	*/
	async delete(params) {
		...
		if (ids) await this.pool.query(`DELETE FROM ${this.computedTableName} WHERE ${this.idColumnName} IN (?) ${this.collectionId ? " AND collection_id = ?" : ""}`, [ids, this.collectionId]);
			...
			await this.pool.query(`DELETE FROM ${this.computedTableName} WHERE ${filterPart} ${this.collectionId ? " AND collection_id = ?" : ""}`, [...params, this.collectionId]);
		}
	}
