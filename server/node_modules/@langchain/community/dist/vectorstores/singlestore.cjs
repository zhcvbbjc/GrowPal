...
var singlestore_exports = /* @__PURE__ */ require_runtime.__exportAll({ SingleStoreVectorStore: () => SingleStoreVectorStore });
...
* functions.
*/
var SingleStoreVectorStore = class SingleStoreVectorStore extends _langchain_core_vectorstores.VectorStore {
	connectionPool;
	tableName;
	idColumnName;
	contentColumnName;
	...
	*/
	async similaritySearchTextAndVectorWithScore(query, vector, k, filter) {
		...
		for (const row of rows) {
			const rowData = row;
			result.push([new _langchain_core_documents.Document({
