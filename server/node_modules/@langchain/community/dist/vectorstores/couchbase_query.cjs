...
var couchbase_query_exports = /* @__PURE__ */ require_runtime.__exportAll({
...
*/
let DistanceStrategy = /* @__PURE__ */ function(DistanceStrategy) {
...
let IndexType = /* @__PURE__ */ function(IndexType) {
	IndexType["COMPOSITE"] = "composite";
	IndexType["HYPERSCALE"] = "hyperscale";
	return IndexType;
}({});
/**
...
*/
var CouchbaseQueryVectorStore = class CouchbaseQueryVectorStore extends _langchain_core_vectorstores.VectorStore {
	...
	*/
	async checkScopeAndCollectionExists() {
		try {
			const scope = (await this._bucket.collections().getAllScopes()).find((s) => s.name === this.scopeName);
			if (!scope) throw new Error(`Scope ${this.scopeName} does not exist`);
			if (!scope.collections.find((c) => c.name === this.collectionName)) throw new Error(`Collection ${this.collectionName} does not exist`);
			return true;
		} catch (err) {
	...
	*/
	async addVectors(vectors, documents, options) {
		...
		for (let index = 0; index < vectors.length; index += 1) {
			...
			const documentToInsert = { [documentIds[index]]: {
		...
		return await this.upsertDocuments(documentsToInsert);
	}
	/**
	...
	*/
	async similaritySearchWithScore(query, k = 4, filter = {}) {
		const queryEmbeddings = await this.embeddings.embedQuery(query);
		return await this.similaritySearchVectorWithScore(queryEmbeddings, k, filter);
	}
	/**
	...
	*/
	async createIndex(options) {
		...
		if (!vectorDim) try {
			vectorDim = (await this.embeddings.embedQuery("check the size of the vector embeddings")).length;
		} catch (e) {
