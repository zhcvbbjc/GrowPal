...
var cassandra_exports = /* @__PURE__ */ require_runtime.__exportAll({ CassandraStore: () => CassandraStore });
/**
...
* texts or documents.
*/
var CassandraStore = class CassandraStore extends _langchain_core_vectorstores.VectorStore {
	table;
	idColumnAutoName = "id";
	...
	_cleanArgs(args) {
		...
		const nonKeyColumnsArg = [...[..._toArray(nonKeyColumns || []), ..._toArray(metadataColumns || [])].filter((col, index, self) => self.findIndex((c) => c.name === col.name) === index)];
		...
		const addDefaultNonKeyColumnIfNeeded = (defaultColumn) => {
			if (!nonKeyColumnsArg.find((col) => col.name === defaultColumn.name)) nonKeyColumnsArg.push(defaultColumn);
		};
		...
		if (!indicesArg.some((index) => new RegExp(`\\(\\s*${this.vectorColumnName.toLowerCase()}\\s*\\)`).test(index.value.toLowerCase()))) indicesArg.push({
	...
	_getColumnByName(columns, columnName) {
		const column = (Array.isArray(columns) ? columns : [columns]).find((col) => col.name === columnName);
	...
	constructor(embeddings, args) {
		...
		this.table = new require_utils_cassandra.CassandraTable(cleanedArgs);
	}
	/**
	...
	*/
	async addVectors(vectors, documents) {
		...
		const values = vectors.map((vector, index) => {
			...
			if (this.idColumnAutoGenerated && (docMetadata[this.idColumnAutoName] === void 0 || docMetadata[this.idColumnAutoName] === "")) docMetadata[this.idColumnAutoName] = (0, uuid.v4)();
			const row = [];
	...
	*/
	async search(query, k, filter, includeEmbedding) {
		...
		return (await this.table.select(queryCols, filter, [orderBy], k))?.rows.map((row) => {
	...
	*/
	async maxMarginalRelevanceSearch(query, options) {
		...
		return (0, _langchain_core_utils_math.maximalMarginalRelevance)(queryEmbedding, queryResults.map((doc) => doc[0].metadata[this.embeddingColumnAlias]), lambda, k).map((idx) => {
