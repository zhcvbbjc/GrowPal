...
var clickhouse_exports = /* @__PURE__ */ require_runtime.__exportAll({ ClickHouseStore: () => ClickHouseStore });
/**
...
* texts or documents.
*/
var ClickHouseStore = class ClickHouseStore extends _langchain_core_vectorstores.VectorStore {
	client;
	indexType;
	indexParam;
	indexQueryParams;
	...
	constructor(embeddings, args) {
		...
		this.client = (0, _clickhouse_client.createClient)({
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
		...
		return (await (await this.client.query({ query: queryStr })).json()).data.map((item) => [new _langchain_core_documents.Document({
