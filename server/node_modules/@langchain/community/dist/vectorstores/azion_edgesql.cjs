...
let azion_sql = require("azion/sql");
let _langchain_core_vectorstores = require("@langchain/core/vectorstores");
//#region src/vectorstores/azion_edgesql.ts
var azion_edgesql_exports = /* @__PURE__ */ require_runtime.__exportAll({ AzionVectorStore: () => AzionVectorStore });
/**
* Example usage:
* ```ts
...
* ```
*/
var AzionVectorStore = class AzionVectorStore extends _langchain_core_vectorstores.VectorStore {
	...
	*/
	createInsertChunks(statements) {
		...
		if (this.getStringBytes(originalStatements.join(" ")) < maxMbSize && originalStatements.length < maxChunkLength) return [originalStatements];
		console.log("Total size exceeded max size. Initiating chunking...");
		let array = [];
		while (originalStatements.length > 0) for (const statement of originalStatements) if (this.getStringBytes(statement) + this.getStringBytes(array.join(" ")) > maxMbSize || array.length + 1 > maxChunkLength) {
	...
	*/
	async similaritySearchVectorWithScore(vector, k, filter, metadataItems) {
      ...
      WHERE ${filters} rowid IN vector_top_k('${this.tableName}_idx', vector('[${vector}]'), ${k})`;
		...
		return this.mapSearches(searches);
	}
	/**
	...
	*/
	mapSearches(searches) {
		return searches.map((resp) => [new _langchain_core_documents.Document({
	...
	*/
	createInsertString(columnNames, values) {
		if (this.expandedMetadata) return `INSERT INTO ${this.tableName} (${columnNames.join(", ")}) 
      VALUES (${values.map((value, index) => columnNames[index] === "embedding" ? `vector('[${value}]')` : `'${this.escapeQuotes(value)}'`).join(", ")})`;
		return `INSERT INTO ${this.tableName} (${columnNames.join(", ")}) 
