...
var opensearch_exports = /* @__PURE__ */ require_runtime.__exportAll({ OpenSearchVectorStore: () => OpenSearchVectorStore });
/**
...
* OpenSearch index.
*/
var OpenSearchVectorStore = class OpenSearchVectorStore extends _langchain_core_vectorstores.VectorStore {
	client;
	indexName;
	isAoss;
	engine;
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
		...
		return body.hits.hits.map((hit) => [new _langchain_core_documents.Document({
	...
	*/
	...
	async ensureIndexExists(dimension, engine = "nmslib", spaceType = "l2", efSearch = 512, efConstruction = 512, numberOfShards = 5, numberOfReplicas = 1, m = 16) {
		...
		if (await this.doesIndexExist()) return;
	...
	*/
	async doesIndexExist() {
		try {
			...
			if (err instanceof _opensearch_project_opensearch.errors.ResponseError && err.statusCode === 404) return false;
			throw err;
		}
	}
	/**
...
Object.defineProperty(exports, "opensearch_exports", {
	enumerable: true,
	get: function() {
		return opensearch_exports;
	}
});
