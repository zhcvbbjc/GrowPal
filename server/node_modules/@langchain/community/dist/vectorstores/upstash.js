...
var upstash_exports = /* @__PURE__ */ __exportAll({ UpstashVectorStore: () => UpstashVectorStore });
...
var UpstashVectorStore = class extends VectorStore {
	...
	*/
	async addVectors(vectors, documents, options) {
		...
		const batchRequests = chunkArray(upstashVectors, CONCURRENT_UPSERT_LIMIT).map((chunk) => this.caller.call(async () => namespace.upsert(chunk)));
	...
	*/
	async _addData(documents, options) {
		...
		const namespace = this.index.namespace(this.namespace ?? "");
		const batchRequests = chunkArray(upstashVectorsWithData, CONCURRENT_UPSERT_LIMIT).map((chunk) => this.caller.call(async () => namespace.upsert(chunk)));
		await Promise.all(batchRequests);
		return documentIds;
	}
	/**
