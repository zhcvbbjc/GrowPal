...
const require_sqlite_where_builder = require("../utils/sqlite_where_builder.cjs");
...
var libsql_exports = /* @__PURE__ */ require_runtime.__exportAll({ LibSQLVectorStore: () => LibSQLVectorStore });
/**
* A vector store using LibSQL/Turso for storage and retrieval.
*/
var LibSQLVectorStore = class LibSQLVectorStore extends _langchain_core_vectorstores.VectorStore {
	db;
	table;
	column;
	/**
	...
	*/
	async addVectors(vectors, documents) {
		...
		for (let i = 0; i < rows.length; i += batchSize) {
			const insertQueries = rows.slice(i, i + batchSize).map((row) => ({
	...
	async similaritySearchVectorWithScore(query, k, filter) {
      ...
      JOIN ${this.table} ON top_k.rowid = ${this.table}.rowid`,
		...
		if (typeof filter === "string") sql.sql += ` AND ${filter}`;
			...
			const where = new require_sqlite_where_builder.SqliteWhereBuilder(filter).buildWhereClause();
		...
		return (await this.db.execute(sql)).rows.map((row) => {
