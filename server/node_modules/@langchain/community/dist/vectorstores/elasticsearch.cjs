...
var elasticsearch_exports = /* @__PURE__ */ require_runtime.__exportAll({
...
* ```
*/
var ElasticVectorSearch = class ElasticVectorSearch extends _langchain_core_vectorstores.VectorStore {
	client;
	indexName;
	engine;
	similarity;
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
		...
		})).hits.hits.map((hit) => [new _langchain_core_documents.Document({
	...
	*/
	static async fromExistingIndex(embeddings, dbConfig) {
		const store = new ElasticVectorSearch(embeddings, dbConfig);
		if (await store.doesIndexExist()) return store;
