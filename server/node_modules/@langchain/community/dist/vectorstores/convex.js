...
var convex_exports = /* @__PURE__ */ __exportAll({ ConvexVectorStore: () => ConvexVectorStore });
/**
...
var ConvexVectorStore = class ConvexVectorStore extends VectorStore {
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
		...
		return (await Promise.all(idsAndScores.map(({ _id }) => this.ctx.runQuery(this.get, { id: _id })))).map(({ [this.textField]: text, [this.embeddingField]: embedding, [this.metadataField]: metadata }, idx) => [new Document({
			pageContent: text,
			metadata: {
				...metadata,
