...
var pgvector_exports = /* @__PURE__ */ require_runtime.__exportAll({ PGVectorStore: () => PGVectorStore });
/**
...
* <br />
*/
var PGVectorStore = class PGVectorStore extends _langchain_core_vectorstores.VectorStore {
	tableName;
	collectionTableName;
	...
	*/
	...
	constructor(embeddings, config) {
		...
		const langchainVerbose = (0, _langchain_core_utils_env.getEnvironmentVariable)("LANGCHAIN_VERBOSE");
	...
	*/
	async getOrCreateCollection() {
      ...
      WHERE name = $1;
    `;
		let collectionId = (await this.pool.query(queryString, [this.collectionName])).rows[0]?.uuid;
		if (!collectionId) {
			const insertString = `
        ...
        RETURNING uuid;
      `;
			collectionId = (await this.pool.query(insertString, [this.collectionName, this.collectionMetadata])).rows[0]?.uuid;
		}
		return collectionId;
	}
	/**
	...
	*/
	buildFilterClauses(filter, paramOffset = 0) {
		...
		for (const [key, value] of Object.entries(filter)) if (typeof value === "object" && value !== null) {
			...
			for (const [opKey, sqlOp] of Object.entries({
				gt: ">",
				gte: ">=",
				lt: "<",
				lte: "<="
			})) if (Object.prototype.hasOwnProperty.call(_value, opKey) && typeof _value[opKey] === "number") {
				paramCount += 1;
	...
	*/
	async maxMarginalRelevanceSearch(query, options) {
		...
		return (0, _langchain_core_utils_math.maximalMarginalRelevance)(queryEmbedding, docs.map((doc) => JSON.parse(doc[0].metadata[this.vectorColumnName])), lambda, k).map((index) => {
