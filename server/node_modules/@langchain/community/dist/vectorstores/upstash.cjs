...
var upstash_exports = /* @__PURE__ */ require_runtime.__exportAll({ UpstashVectorStore: () => UpstashVectorStore });
...
*/
var UpstashVectorStore = class extends _langchain_core_vectorstores.VectorStore {
	index;
	caller;
	...
	constructor(embeddings, args) {
		...
		if (embeddings instanceof _langchain_core_utils_testing.FakeEmbeddings) this.useUpstashEmbeddings = true;
	...
	*/
	async addVectors(vectors, documents, options) {
		...
		const batchRequests = (0, _langchain_core_utils_chunk_array.chunkArray)(upstashVectors, CONCURRENT_UPSERT_LIMIT).map((chunk) => this.caller.call(async () => namespace.upsert(chunk)));
	...
	*/
	async _addData(documents, options) {
		...
		const namespace = this.index.namespace(this.namespace ?? "");
		const batchRequests = (0, _langchain_core_utils_chunk_array.chunkArray)(upstashVectorsWithData, CONCURRENT_UPSERT_LIMIT).map((chunk) => this.caller.call(async () => namespace.upsert(chunk)));
		await Promise.all(batchRequests);
		return documentIds;
	}
	/**
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
		return (await this._runUpstashQuery(query, k, filter)).map((res) => {
	...
	*/
	static async fromTexts(texts, metadatas, embeddings, dbConfig) {
		...
		for (let i = 0; i < texts.length; i += 1) {
			...
			const newDocument = new _langchain_core_documents.Document({
				pageContent: texts[i],
				metadata
			});
