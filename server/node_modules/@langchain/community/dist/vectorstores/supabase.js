...
var supabase_exports = /* @__PURE__ */ __exportAll({ SupabaseVectorStore: () => SupabaseVectorStore });
/**
...
var SupabaseVectorStore = class SupabaseVectorStore extends VectorStore {
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
		return (await this._searchSupabase(query, k, filter)).map((resp) => [new Document({
	...
	*/
	async maxMarginalRelevanceSearch(query, options) {
		...
		return maximalMarginalRelevance(queryEmbedding, searches.map((searchResp) => searchResp.embedding), options.lambda, options.k).map((idx) => new Document({
