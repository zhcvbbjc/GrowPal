...
let _lancedb_lancedb = require("@lancedb/lancedb");
//#region src/vectorstores/lancedb.ts
var lancedb_exports = /* @__PURE__ */ require_runtime.__exportAll({ LanceDB: () => LanceDB });
/**
...
* embeddings.
*/
var LanceDB = class LanceDB extends _langchain_core_vectorstores.VectorStore {
	table;
	textKey;
	uri;
	tableName;
	...
	*/
	async addVectors(vectors, documents) {
		...
		if (!this.table) {
			this.table = await (await (0, _lancedb_lancedb.connect)(this.uri)).createTable(this.tableName, data, { mode: this.mode });
			return;
		}
	...
	*/
	async similaritySearch(query, k) {
		return (await this.similaritySearchWithScore(query, k)).map((result) => result[0]);
	}
	/**
	* Performs a similarity search in the database and returns
	...
	*/
	async similaritySearchVectorWithScore(query, k) {
		...
		results.forEach((item) => {
			...
			docsAndScore.push([new _langchain_core_documents.Document({
