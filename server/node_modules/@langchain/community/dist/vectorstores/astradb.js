...
var astradb_exports = /* @__PURE__ */ __exportAll({ AstraDBVectorStore: () => AstraDBVectorStore });
var AstraDBVectorStore = class AstraDBVectorStore extends VectorStore {
	...
	constructor(embeddings, args) {
		...
		this.astraDBClient = new DataAPIClient(token, { caller: ["langchainjs"] }).db(endpoint, { keyspace });
	...
	*/
	async maxMarginalRelevanceSearch(query, options) {
		...
		const topMmrMatches = maximalMarginalRelevance(queryEmbedding, results.map((row) => row.$vector), options.lambda, options.k).map((idx) => results[idx]);
		const docs = [];
