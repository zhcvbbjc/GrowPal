...
var chroma_exports = /* @__PURE__ */ require_runtime.__exportAll({ Chroma: () => Chroma });
/**
...
* <br />
*/
var Chroma = class Chroma extends _langchain_core_vectorstores.VectorStore {
	index;
	collection;
	collectionName;
	collectionMetadata;
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
		...
		const { ids, distances, documents, metadatas } = await (await this.ensureCollection()).query({
		...
		for (let i = 0; i < firstIds.length; i += 1) {
			...
			results.push([new _langchain_core_documents.Document({
