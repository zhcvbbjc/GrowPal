...
var zep_exports = /* @__PURE__ */ require_runtime.__exportAll({ ZepVectorStore: () => ZepVectorStore });
/**
...
*/
var ZepVectorStore = class ZepVectorStore extends _langchain_core_vectorstores.VectorStore {
	client;
	collection;
	initPromise;
	autoEmbed = false;
	constructor(embeddings, args) {
		...
		if (this.embeddings instanceof _langchain_core_utils_testing.FakeEmbeddings) this.autoEmbed = true;
	...
	*/
	async initCollection(args) {
		this.client = await _getzep_zep_js.ZepClient.init(args.apiUrl, args.apiKey);
		try {
 ...
 is not set to auto-embed.`);
			...
			if (err instanceof Error) if (typeof _getzep_zep_js.NotFoundError === "function" && err instanceof _getzep_zep_js.NotFoundError || err.name === "NotFoundError") await this.createCollection(args);
			else throw err;
		}
	}
	/**
 ...
 You can create a new Collection by providing embeddingDimensions.`);
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
		await this.initPromise;
		return zepDocsToDocumentsAndScore(await this.collection.search({
	...
	*/
	async similaritySearch(query, k = 4, filter = void 0, _callbacks = void 0) {
		await this.initPromise;
		let results;
		if (this.autoEmbed) results = zepDocsToDocumentsAndScore(await this.collection.search({
			text: query,
			metadata: assignMetadata(filter)
		}, k));
	...
	*/
	async maxMarginalRelevanceSearch(query, options) {
		...
		return (0, _langchain_core_utils_math.maximalMarginalRelevance)(queryEmbedding, embeddingList, lambda, k).filter((idx) => idx !== -1).map((idx) => results[idx][0]);
	}
	/**
