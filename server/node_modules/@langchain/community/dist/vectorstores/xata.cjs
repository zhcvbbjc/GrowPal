...
var xata_exports = /* @__PURE__ */ require_runtime.__exportAll({ XataVectorSearch: () => XataVectorSearch });
/**
...
*/
var XataVectorSearch = class extends _langchain_core_vectorstores.VectorStore {
	client;
	table;
	_vectorstoreType() {
		return "xata";
	...
	*/
	async addVectors(vectors, documents, options) {
		...
		return (await this.client.db[this.table].createOrReplace(rows)).map((row) => row.id);
	}
	/**
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
		...
		return records?.map((record) => [new _langchain_core_documents.Document({
