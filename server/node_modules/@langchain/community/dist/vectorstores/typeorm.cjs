...
var typeorm_exports = /* @__PURE__ */ require_runtime.__exportAll({
...
*/
var TypeORMVectorStoreDocument = class extends _langchain_core_documents.Document {
	embedding;
};
...
*/
var TypeORMVectorStore = class TypeORMVectorStore extends _langchain_core_vectorstores.VectorStore {
	tableName;
	schemaName;
	documentEntity;
	filter;
	...
	constructor(embeddings, fields) {
		...
		const TypeORMDocumentEntity = new typeorm.EntitySchema({
		...
		this._verbose = fields.verbose ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("LANGCHAIN_VERBOSE") === "true";
	}
	/**
	...
	async similaritySearchVectorWithScore(query, k, filter) {
      ...
      LIMIT $3;`;
		...
		for (const doc of documents) if (doc._distance != null && doc.pageContent != null) {
			const document = new _langchain_core_documents.Document(doc);
			document.id = doc.id;
	...
	*/
	static async fromExistingIndex(embeddings, dbConfig) {
		return await TypeORMVectorStore.fromDataSource(embeddings, dbConfig);
	}
};
//#endregion
exports.TypeORMVectorStore = TypeORMVectorStore;
exports.TypeORMVectorStoreDocument = TypeORMVectorStoreDocument;
Object.defineProperty(exports, "typeorm_exports", {
	enumerable: true,
	get: function() {
		return typeorm_exports;
	}
});
