...
let pg_copy_streams = require("pg-copy-streams");
let node_stream_promises = require("node:stream/promises");
//#region src/vectorstores/analyticdb.ts
var analyticdb_exports = /* @__PURE__ */ require_runtime.__exportAll({ AnalyticDBVectorStore: () => AnalyticDBVectorStore });
...
*/
var AnalyticDBVectorStore = class AnalyticDBVectorStore extends _langchain_core_vectorstores.VectorStore {
	pool;
	embeddingDimension;
	collectionName;
	preDeleteCollection;
	...
	*/
	async createTableIfNotExists() {
      ...
      `;
			if ((await client.query(indexQuery)).rowCount === 0) {
		...
		try {
			...
			for (let i = 0; i < documents.length; i += 1) {
				...
				if (chunksTableData.length === chunkSize) {
					...
					await (0, node_stream_promises.pipeline)(rs, client.query((0, pg_copy_streams.from)(`COPY ${this.collectionName}(id, embedding, document, metadata) FROM STDIN`)));
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
		...
		if (filterEntries.length > 0) filterCondition = `WHERE ${filterEntries.map((_, index) => `metadata->>$${2 * index + 3} = $${2 * index + 4}`).join(" AND ")}`;
		const sqlQuery = `
    ...
    `;
		...
		return rows.map((row) => [new _langchain_core_documents.Document({
