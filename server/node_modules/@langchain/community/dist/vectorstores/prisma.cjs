...
var prisma_exports = /* @__PURE__ */ require_runtime.__exportAll({ PrismaVectorStore: () => PrismaVectorStore });
...
*/
var PrismaVectorStore = class PrismaVectorStore extends _langchain_core_vectorstores.VectorStore {
	tableName;
	vectorColumnName;
	selectColumns;
	filter;
	...
	*/
	async addDocumentsWithVectors(vectors, documents) {
		const tableNameRaw = this.Prisma.raw(`"${this.tableName}"`);
		const vectorColumnRaw = this.Prisma.raw(`"${this.vectorColumnName}"`);
		const allColumns = [...this.selectColumns.map((col) => this.Prisma.raw(`"${col}"`)), vectorColumnRaw];
		await this.db.$transaction(vectors.map((vector, idx) => {
			const document = documents[idx];
			const vectorString = `[${vector.join(",")}]`;
			const allValues = [...this.selectColumns.map((col) => {
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
		...
		const results = [];
		for (const article of articles) if (article._distance != null && article[this.contentColumn] != null) results.push([new _langchain_core_documents.Document({
