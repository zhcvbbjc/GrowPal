...
var vectara_exports = /* @__PURE__ */ require_runtime.__exportAll({
...
* class.
*/
var VectaraStore = class VectaraStore extends _langchain_core_vectorstores.VectorStore {
	get lc_secrets() {
		return {
	...
	constructor(args) {
		super(new _langchain_core_utils_testing.FakeEmbeddings(), args);
		const apiKey = args.apiKey ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("VECTARA_API_KEY");
		if (!apiKey) throw new Error("Vectara api key is not provided.");
		this.apiKey = apiKey;
		this.source = args.source ?? "langchainjs";
		const corpusId = args.corpusId ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("VECTARA_CORPUS_ID")?.split(",").map((id) => {
			const num = Number(id);
		...
		const customerId = args.customerId ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("VECTARA_CUSTOMER_ID");
	...
	async addFiles(files, metadatas = void 0) {
		...
		for (const [index, file] of files.entries()) {
			...
			if (status === 409) throw new Error(`File at index ${index} already exists in Vectara`);
				...
				const doc_id = (await response.json()).document.documentId;
				doc_ids.push(doc_id);
			}
		}
	...
	*/
	async vectaraQuery(query, k, vectaraFilterObject, summary = {
		...
		return {
			documents: responses.map((response) => new _langchain_core_documents.Document({
	...
	*/
	async similaritySearchWithScore(query, k, filter) {
		...
		return summaryResult.documents.map((document, index) => [document, summaryResult.scores[index]]);
	}
	/**
	...
	*/
	async similaritySearch(query, k, filter) {
		return (await this.similaritySearchWithScore(query, k || 10, filter || DEFAULT_FILTER)).map((result) => result[0]);
	}
	/**
	...
	*/
	static fromTexts(texts, metadatas, _embeddings, args) {
		...
		return VectaraStore.fromDocuments(docs, new _langchain_core_utils_testing.FakeEmbeddings(), args);
	}
	/**
