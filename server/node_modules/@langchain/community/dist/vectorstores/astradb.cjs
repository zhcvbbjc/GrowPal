...
var astradb_exports = /* @__PURE__ */ require_runtime.__exportAll({ AstraDBVectorStore: () => AstraDBVectorStore });
var AstraDBVectorStore = class AstraDBVectorStore extends _langchain_core_vectorstores.VectorStore {
	astraDBClient;
	collectionName;
	collection;
	collectionOptions;
	...
	constructor(embeddings, args) {
		...
		this.astraDBClient = new _datastax_astra_db_ts.DataAPIClient(token, { caller: ["langchainjs"] }).db(endpoint, { keyspace });
		...
		this.caller = new _langchain_core_utils_async_caller.AsyncCaller(callerArgs);
	...
	*/
	async maxMarginalRelevanceSearch(query, options) {
		...
		const results = await (await this.collection.find(options.filter ?? {}, {
			sort: { $vector: queryEmbedding },
			limit: options.k,
			includeSimilarity: true
		})).toArray() ?? [];
		const topMmrMatches = (0, _langchain_core_utils_math.maximalMarginalRelevance)(queryEmbedding, results.map((row) => row.$vector), options.lambda, options.k).map((idx) => results[idx]);
		const docs = [];
