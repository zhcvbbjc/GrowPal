...
var zep_exports = /* @__PURE__ */ __exportAll({ ZepVectorStore: () => ZepVectorStore });
/**
...
var ZepVectorStore = class ZepVectorStore extends VectorStore {
 ...
 is not set to auto-embed.`);
			...
			if (err instanceof Error) if (typeof NotFoundError === "function" && err instanceof NotFoundError || err.name === "NotFoundError") await this.createCollection(args);
			else throw err;
		}
	}
	/**
 ...
 You can create a new Collection by providing embeddingDimensions.`);
	...
	*/
	async maxMarginalRelevanceSearch(query, options) {
		...
		return maximalMarginalRelevance(queryEmbedding, embeddingList, lambda, k).filter((idx) => idx !== -1).map((idx) => results[idx][0]);
	}
	/**
