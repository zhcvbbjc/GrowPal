...
var pgvector_exports = /* @__PURE__ */ __exportAll({ PGVectorStore: () => PGVectorStore });
/**
...
var PGVectorStore = class PGVectorStore extends VectorStore {
	...
	*/
	...
	constructor(embeddings, config) {
		...
		this.pool = config.pool ?? new pg.Pool(config.postgresConnectionOptions);
	...
	*/
	async maxMarginalRelevanceSearch(query, options) {
		...
		return maximalMarginalRelevance(queryEmbedding, docs.map((doc) => JSON.parse(doc[0].metadata[this.vectorColumnName])), lambda, k).map((index) => {
