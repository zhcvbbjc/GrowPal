...
var convex_exports = /* @__PURE__ */ require_runtime.__exportAll({ ConvexVectorStore: () => ConvexVectorStore });
/**
...
*/
var ConvexVectorStore = class ConvexVectorStore extends _langchain_core_vectorstores.VectorStore {
	ctx;
	table;
	index;
	textField;
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
		...
		return (await Promise.all(idsAndScores.map(({ _id }) => this.ctx.runQuery(this.get, { id: _id })))).map(({ [this.textField]: text, [this.embeddingField]: embedding, [this.metadataField]: metadata }, idx) => [new _langchain_core_documents.Document({
			pageContent: text,
			metadata: {
				...metadata,
	...
	*/
	static async fromTexts(texts, metadatas, embeddings, dbConfig) {
		const docs = texts.map((text, i) => new _langchain_core_documents.Document({
			pageContent: text,
