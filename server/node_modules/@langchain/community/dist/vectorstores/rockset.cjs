...
let _rockset_client_dist_codegen_api_js = require("@rockset/client/dist/codegen/api.js");
//#region src/vectorstores/rockset.ts
var rockset_exports = /* @__PURE__ */ require_runtime.__exportAll({
...
*/
var RocksetStore = class RocksetStore extends _langchain_core_vectorstores.VectorStore {
	client;
	collectionName;
	workspaceName;
	textKey;
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
		...
		} })).results?.map((rocksetDoc) => [new _langchain_core_documents.Document({
	...
	*/
	static async collectionReady(dbConfig) {
		return (await dbConfig.client.collections.getCollection(dbConfig.workspaceName ?? "commons", dbConfig.collectionName)).data?.status === _rockset_client_dist_codegen_api_js.Collection.StatusEnum.READY;
	}
	/**
