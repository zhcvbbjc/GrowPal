...
let _langchain_classic_stores_doc_in_memory = require("@langchain/classic/stores/doc/in_memory");
//#region src/vectorstores/faiss.ts
var faiss_exports = /* @__PURE__ */ require_runtime.__exportAll({ FaissStore: () => FaissStore });
/**
...
* vectors.
*/
var FaissStore = class FaissStore extends _langchain_core_vectorstores.SaveableVectorStore {
	_index;
	_mapping;
	docstore;
	args;
	...
	constructor(embeddings, args) {
		...
		this.docstore = args?.docstore ?? new _langchain_classic_stores_doc_in_memory.SynchronousInMemoryDocstore();
	}
	/**
	...
	*/
	async mergeFrom(targetIndex) {
		...
		if (targetIndexDimensions !== this.index.getDimension()) throw new Error("Cannot merge indexes with different dimensions.");
	...
	*/
	static async load(directory, embeddings) {
		const fs = await import("node:fs/promises");
		const path = await import("node:path");
		const readStore = (directory) => fs.readFile(path.join(directory, "docstore.json"), "utf8").then(JSON.parse);
		...
		const docstore = new _langchain_classic_stores_doc_in_memory.SynchronousInMemoryDocstore(new Map(docstoreFiles));
	...
	static async loadFromPython(directory, embeddings) {
		...
		class PyInMemoryDocstore {
			_dict;
			toInMemoryDocstore() {
				const s = new _langchain_classic_stores_doc_in_memory.SynchronousInMemoryDocstore();
		...
		const readStore = async (directory) => {
			...
			const [rawStore, mapping] = new Parser({ nameResolver: new NameRegistry().register("langchain.docstore.in_memory", "InMemoryDocstore", PyInMemoryDocstore).register("langchain_community.docstore.in_memory", "InMemoryDocstore", PyInMemoryDocstore).register("langchain.schema", "Document", PyDocument).register("langchain.docstore.document", "Document", PyDocument).register("langchain.schema.document", "Document", PyDocument).register("langchain_core.documents.base", "Document", PyDocument).register("pathlib", "WindowsPath", (...args) => args.join("\\")).register("pathlib", "PosixPath", (...args) => args.join("/")) }).parse(buffer);
			return {
				store: rawStore.toInMemoryDocstore(),
				mapping
			};
		};
		const readIndex = async (directory) => {
