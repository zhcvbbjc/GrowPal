...
var typesense_exports = /* @__PURE__ */ require_runtime.__exportAll({ Typesense: () => Typesense });
/**
* Typesense vector store.
*/
var Typesense = class Typesense extends _langchain_core_vectorstores.VectorStore {
	client;
	schemaName;
	searchParams;
	vectorColumnName;
	...
	constructor(embeddings, config) {
		...
		this.caller = new _langchain_core_utils_async_caller.AsyncCaller(config);
	}
	/**
	...
	*/
	_typesenseRecordsToDocuments(typesenseRecords) {
		return typesenseRecords?.map((hit) => {
			const objectWithMetadatas = {};
	...
	*/
	async similaritySearchVectorWithScore(vectorPrompt, k, filter = {}) {
		...
		const hits = (await this.client.multiSearch.perform({ searches: [{
			...this.searchParams,
			...filter,
			per_page: amount,
			vector_query,
