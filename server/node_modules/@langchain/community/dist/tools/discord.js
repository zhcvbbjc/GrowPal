...
*/
var DiscordGetMessagesTool = class extends Tool {
	static lc_name() {
  ...
  permissions for the channel.`;
	...
	constructor(fields) {
		...
		this.client = client ?? new Client({ intents: [GatewayIntentBits.Guilds, GatewayIntentBits.GuildMessages] });
...
* variables.
*/
var DiscordGetGuildsTool = class extends Tool {
	static lc_name() {
...
*/
var DiscordGetTextChannelsTool = class extends Tool {
	static lc_name() {
	...
	async _call(input) {
		try {
			...
			await this.client.destroy();
			const results = channels.filter((channel) => channel?.type === ChannelType.GuildText).map((channel) => ({
				id: channel?.id,
				name: channel?.name,
...
*/
var DiscordSendMessagesTool = class extends Tool {
	static lc_name() {
  ...
  Input should be the discord channel message, since we will already have the channel ID.`;
	...
	async _call(message) {
		try {
			...
			if (!(channel.constructor === TextChannel)) throw new Error("Channel is not text channel, cannot send message");
...
* message content, author, and time the message was created for each message.
*/
var DiscordChannelSearchTool = class extends Tool {
	static lc_name() {
...
//#endregion
export { DiscordChannelSearchTool, DiscordGetGuildsTool, DiscordGetMessagesTool, DiscordGetTextChannelsTool, DiscordSendMessagesTool, discord_exports };
