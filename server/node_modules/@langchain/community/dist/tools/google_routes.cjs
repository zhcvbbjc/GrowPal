...
var google_routes_exports = /* @__PURE__ */ require_runtime.__exportAll({ GoogleRoutesAPI: () => GoogleRoutesAPI });
...
const defaultGoogleRoutesSchema = zod_v3.z.object({
	origin: zod_v3.z.string().describe(`Origin address, can be either a place or an address.`),
	destination: zod_v3.z.string().describe(`Destination address, can be either a place or an address.`),
	travel_mode: zod_v3.z.enum([
		"DRIVE",
		"WALK",
		"BICYCLE",
		"TRANSIT",
	...
	computeAlternativeRoutes: zod_v3.z.boolean().describe(`Compute alternative routes, set to true if user wants multiple routes, false otherwise.`),
	departureTime: zod_v3.z.string().optional().describe(`Time that the user wants to depart.
      ...
      `),
	arrivalTime: zod_v3.z.string().optional().describe(`Time that the user wants to arrive.
	...
	transitPreferences: zod_v3.z.object({ routingPreference: zod_v3.z.enum(["LESS_WALKING", "FEWER_TRANSFERS"]).describe(`Transit routing preference.
        By default, it should not be included.`) }).optional().describe(`Transit routing preference.
       By default, it should not be included.`),
	extraComputations: zod_v3.z.array(zod_v3.z.enum(["TOLLS"])).optional().describe(`Calculate tolls for the route.`)
});
/**
* Class for interacting with the Google Routes API
* It extends the StructuredTool class to perform retrieval.
*/
var GoogleRoutesAPI = class extends _langchain_core_tools.StructuredTool {
	static lc_name() {
	...
	constructor(fields) {
		super(...arguments);
		const apiKey = fields?.apiKey ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("GOOGLE_ROUTES_API_KEY");
...
Object.defineProperty(exports, "google_routes_exports", {
	enumerable: true,
	get: function() {
		return google_routes_exports;
	}
});

//# sourceMappingURL=google_routes.cjs.map