...
var google_scholar_exports = /* @__PURE__ */ require_runtime.__exportAll({ SERPGoogleScholarAPITool: () => SERPGoogleScholarAPITool });
/**
* Tool for querying Google Scholar using the SerpApi service.
*/
var SERPGoogleScholarAPITool = class extends _langchain_core_tools.Tool {
	/**
  ...
  articles and papers by keywords or authors. Input should be a search query string.`;
	...
	*/
	constructor(fields) {
		super(...arguments);
		const apiKey = fields?.apiKey ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("SERPAPI_API_KEY");
	...
	*/
	async _call(input) {
		...
		const results = (await response.json()).organic_results?.map((item) => ({
			title: item.title,
			link: item.link,
...
Object.defineProperty(exports, "google_scholar_exports", {
	enumerable: true,
	get: function() {
		return google_scholar_exports;
	}
});

//# sourceMappingURL=google_scholar.cjs.map