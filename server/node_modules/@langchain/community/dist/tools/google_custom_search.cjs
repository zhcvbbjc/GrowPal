...
var google_custom_search_exports = /* @__PURE__ */ require_runtime.__exportAll({ GoogleCustomSearch: () => GoogleCustomSearch });
/**
...
* be set.
*/
var GoogleCustomSearch = class extends _langchain_core_tools.Tool {
	static lc_name() {
	...
	constructor(fields = {
		apiKey: (0, _langchain_core_utils_env.getEnvironmentVariable)("GOOGLE_API_KEY"),
		googleCSEId: (0, _langchain_core_utils_env.getEnvironmentVariable)("GOOGLE_CSE_ID")
	}) {
		super(...arguments);
	...
	async _call(input) {
		...
		const results = (await res.json())?.items?.map((item) => ({
			title: item.title,
			link: item.link,
