...
var wikipedia_query_run_exports = /* @__PURE__ */ require_runtime.__exportAll({ WikipediaQueryRun: () => WikipediaQueryRun });
/**
...
* </details>
*/
var WikipediaQueryRun = class extends _langchain_core_tools.Tool {
	static lc_name() {
	...
	*/
	async content(page, redirect = true) {
		try {
			return (await this._fetchPage(page, redirect)).extract;
		} catch (error) {
	...
	*/
	...
	async _fetchPage(page, redirect) {
		...
		if (!response.ok) throw new Error("Network response was not ok");
		const { pages } = (await response.json()).query;
