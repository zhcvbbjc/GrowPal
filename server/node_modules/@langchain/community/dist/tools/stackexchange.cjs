...
var stackexchange_exports = /* @__PURE__ */ require_runtime.__exportAll({ StackExchangeAPI: () => StackExchangeAPI });
/**
* Class for interacting with the StackExchange API
* It extends the base Tool class to perform retrieval.
*/
var StackExchangeAPI = class extends _langchain_core_tools.Tool {
	name = "stackexchange";
	...
	async _call(query) {
		...
		for (const question of questions) {
			...
			if (relevant_answers.length > 0) {
				const { excerpt } = accepted_answers.length > 0 ? accepted_answers[0] : relevant_answers[0];
				res_text += `\nAnswer: ${excerpt}`;
			}
...
Object.defineProperty(exports, "stackexchange_exports", {
	enumerable: true,
	get: function() {
		return stackexchange_exports;
	}
});

//# sourceMappingURL=stackexchange.cjs.map