...
var searchapi_exports = /* @__PURE__ */ require_runtime.__exportAll({ SearchApi: () => SearchApi });
...
* ```
*/
var SearchApi = class extends _langchain_core_tools.Tool {
	static lc_name() {
		return "SearchApi";
	}
	/**
	...
	*/
	...
	apiKey;
	params;
	constructor(apiKey = (0, _langchain_core_utils_env.getEnvironmentVariable)("SEARCHAPI_API_KEY"), params = {}) {
		super(...arguments);
	...
	*/
	buildUrl(searchQuery) {
		...
		return `https://www.searchapi.io/api/v1/search?${new URLSearchParams(preparedParams)}`;
	}
	/** @ignore */
	/**
	* Calls the SearchAPI.
	...
	*/
	async _call(input) {
		const json = await (await fetch(this.buildUrl(input))).json();
		...
		if (json.organic_results) return json.organic_results.filter((r) => r.snippet).map((r) => r.snippet).join("\n");
		if (json.jobs) return json.jobs.slice(0, 1).filter((r) => r.description).map((r) => r.description).join("\n");
		if (json.videos) return json.videos.filter((r) => r.title && r.link).map((r) => `Title: "${r.title}" Link: ${r.link}`).join("\n");
		if (json.images) return json.images.slice(0, 15).filter((r) => r.title && r.original && isJSONObject(r.original) && r.original.link).map((r) => `Title: "${r.title}" Link: ${r.original.link}`).join("\n");
