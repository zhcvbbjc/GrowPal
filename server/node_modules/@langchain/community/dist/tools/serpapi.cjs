...
var serpapi_exports = /* @__PURE__ */ require_runtime.__exportAll({ SerpAPI: () => SerpAPI });
/**
* Wrapper around SerpAPI.
*
* To use, you should have the `serpapi` package installed and the SERPAPI_API_KEY environment variable set.
*/
var SerpAPI = class extends _langchain_core_tools.Tool {
	static lc_name() {
		return "SerpAPI";
	}
	toJSON() {
	...
	params;
	baseUrl;
	constructor(apiKey = (0, _langchain_core_utils_env.getEnvironmentVariable)("SERPAPI_API_KEY"), params = {}, baseUrl = "https://serpapi.com") {
		super(...arguments);
	...
	*/
	buildUrl(path, parameters, baseUrl) {
		...
		return `${baseUrl}/${path}?${new URLSearchParams(nonUndefinedParams)}`;
	}
	/** @ignore */
	async _call(input) {
		const { timeout, ...params } = this.params;
		const res = await (await fetch(this.buildUrl("search", {
			...params,
			api_key: this.key,
			q: input
		}, this.baseUrl), { signal: timeout ? AbortSignal.timeout(timeout) : void 0 })).json();
