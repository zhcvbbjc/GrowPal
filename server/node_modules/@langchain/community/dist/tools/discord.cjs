...
var discord_exports = /* @__PURE__ */ require_runtime.__exportAll({
...
*/
var DiscordGetMessagesTool = class extends _langchain_core_tools.Tool {
	static lc_name() {
  ...
  permissions for the channel.`;
	...
	constructor(fields) {
		super();
		const { botToken = (0, _langchain_core_utils_env.getEnvironmentVariable)("DISCORD_BOT_TOKEN"), messageLimit = 10, client } = fields ?? {};
...
* variables.
*/
var DiscordGetGuildsTool = class extends _langchain_core_tools.Tool {
	static lc_name() {
	...
	constructor(fields) {
		super();
		const { botToken = (0, _langchain_core_utils_env.getEnvironmentVariable)("DISCORD_BOT_TOKEN"), client } = fields ?? {};
...
*/
var DiscordGetTextChannelsTool = class extends _langchain_core_tools.Tool {
	static lc_name() {
	...
	async _call(input) {
		try {
			await this.client.login(this.botToken);
			const channels = await (await this.client.guilds.fetch(input)).channels.fetch();
			await this.client.destroy();
...
*/
var DiscordSendMessagesTool = class extends _langchain_core_tools.Tool {
	static lc_name() {
  ...
  Input should be the discord channel message, since we will already have the channel ID.`;
	...
	constructor(fields) {
		super();
		const { botToken = (0, _langchain_core_utils_env.getEnvironmentVariable)("DISCORD_BOT_TOKEN"), channelId = (0, _langchain_core_utils_env.getEnvironmentVariable)("DISCORD_CHANNEL_ID"), client } = fields ?? {};
...
* message content, author, and time the message was created for each message.
*/
var DiscordChannelSearchTool = class extends _langchain_core_tools.Tool {
	static lc_name() {
  ...
  should have read permissions for the channel.`;
	...
	async _call(searchTerm) {
		try {
			...
			await this.client.destroy();
			const results = messages.filter((message) => message.content.toLowerCase().includes(searchTerm.toLowerCase())).map((message) => ({
				author: message.author.tag,
				content: message.content,
				timestamp: message.createdAt
			})) ?? [];
