...
var dataforseo_api_search_exports = /* @__PURE__ */ require_runtime.__exportAll({ DataForSeoAPISearch: () => DataForSeoAPISearch });
/**
* @class DataForSeoAPISearch
* @extends {Tool}
* @description Represents a wrapper class to work with DataForSEO SERP API.
*/
var DataForSeoAPISearch = class extends _langchain_core_tools.Tool {
	static lc_name() {
	...
	*/
	constructor(config = {}) {
		super();
		const apiLogin = config.apiLogin ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("DATAFORSEO_LOGIN");
		const apiPassword = config.apiPassword ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("DATAFORSEO_PASSWORD");
	...
	*/
	prepareRequest(keyword) {
		...
		const headers = {
			Authorization: `Basic ${Buffer.from(`${this.apiLogin}:${this.apiPassword}`, "utf-8").toString("base64")}`,
