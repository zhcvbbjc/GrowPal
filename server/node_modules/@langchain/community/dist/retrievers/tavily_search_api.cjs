...
var tavily_search_api_exports = /* @__PURE__ */ require_runtime.__exportAll({ TavilySearchAPIRetriever: () => TavilySearchAPIRetriever });
/**
* A class for retrieving documents related to a given search term
* using the Tavily Search API.
*/
var TavilySearchAPIRetriever = class extends _langchain_core_retrievers.BaseRetriever {
	static lc_name() {
	...
	constructor(fields) {
		...
		this.apiKey = fields?.apiKey ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("TAVILY_API_KEY");
	...
	async _getRelevantDocuments(query, _runManager) {
		...
		if (this.includeGeneratedAnswer) docs.push(new _langchain_core_documents.Document({
...
Object.defineProperty(exports, "tavily_search_api_exports", {
	enumerable: true,
	get: function() {
		return tavily_search_api_exports;
	}
});

//# sourceMappingURL=tavily_search_api.cjs.map