...
var supabase_exports = /* @__PURE__ */ require_runtime.__exportAll({ SupabaseHybridSearch: () => SupabaseHybridSearch });
/**
...
*/
var SupabaseHybridSearch = class extends _langchain_core_retrievers.BaseRetriever {
	static lc_name() {
	...
	*/
	async similaritySearch(query, k, _callbacks) {
		const matchDocumentsParams = {
			query_embedding: await this.embeddings.embedQuery(query),
			match_count: k
		};
	...
	*/
	...
	async _getRelevantDocuments(query, runManager) {
		return (await this.hybridSearch(query, this.similarityK, this.keywordK, runManager?.getChild("hybrid_search"))).map(([doc]) => doc);
	}
};
//#endregion
