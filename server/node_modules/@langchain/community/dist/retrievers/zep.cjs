...
var zep_exports = /* @__PURE__ */ require_runtime.__exportAll({ ZepRetriever: () => ZepRetriever });
/**
...
* ```
*/
var ZepRetriever = class extends _langchain_core_retrievers.BaseRetriever {
	static lc_name() {
		return "ZepRetriever";
	}
	lc_namespace = [
	...
	constructor(config) {
		...
		this.zepClientPromise = _getzep_zep_js.ZepClient.init(config.url, config.apiKey);
	}
	/**
	...
	*/
	searchMessageResultToDoc(results) {
		return results.filter((r) => r.message).map(({ message: { content, metadata: messageMetadata } = {}, dist, ...rest }) => new _langchain_core_documents.Document({
			pageContent: content ?? "",
			metadata: {
				score: dist,
				...messageMetadata,
	...
	*/
	searchSummaryResultToDoc(results) {
		return results.filter((r) => r.summary).map(({ summary: { content, metadata: summaryMetadata } = {}, dist, ...rest }) => new _langchain_core_documents.Document({
			pageContent: content ?? "",
			metadata: {
				score: dist,
				...summaryMetadata,
	...
	*/
	async _getRelevantDocuments(query) {
		...
		try {
			...
			if (error instanceof _getzep_zep_js.NotFoundError) return Promise.resolve([]);
			throw error;
		}
	}
};
