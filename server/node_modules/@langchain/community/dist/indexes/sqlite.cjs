...
var sqlite_exports = /* @__PURE__ */ require_runtime.__exportAll({ SQLiteRecordManager: () => SQLiteRecordManager });
...
CREATE INDEX IF NOT EXISTS group_id_index ON "${this.tableName}" (group_id);`);
	...
	async getTime() {
		try {
			const { epoch } = this.db.prepare("SELECT strftime('%s', 'now') AS epoch").get();
			return Number(epoch);
		} catch (error) {
...
WHERE namespace = ? AND key IN (${placeholders})`;
	...
	async listKeys(options) {
		...
		query += ";";
		try {
			return this.db.prepare(query).all(...values).map((row) => row.key);
		} catch (error) {
