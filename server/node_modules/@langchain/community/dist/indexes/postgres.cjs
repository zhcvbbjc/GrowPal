...
var postgres_exports = /* @__PURE__ */ require_runtime.__exportAll({ PostgresRecordManager: () => PostgresRecordManager });
	...
	*/
	...
	async exists(keys) {
      ...
      WITH ordered_keys AS (
        SELECT * FROM unnest(ARRAY[${keys.map((_, i) => `$${i + startIndex}`).join(", ")}]) WITH ORDINALITY as t(key, o)
      )
      ...
      ORDER BY ok.o;
      `;
		return (await this.pool.query(query, [this.namespace, ...keys.flat()])).rows.map((row) => row.ex);
	}
	async listKeys(options) {
		...
		query += ";";
		return (await this.pool.query(query, values)).rows.map((row) => row.key);
	}
