...
function _ensureMessageContents(messages) {
	const updatedMsgs = [];
	for (const message of messages) if (_langchain_core_messages.ToolMessage.isInstance(message)) if (typeof message.content === "string") {
	...
	else if (_langchain_core_messages.SystemMessage.isInstance(message) || _langchain_core_messages.HumanMessage.isInstance(message) || _langchain_core_messages.AIMessage.isInstance(message)) updatedMsgs.push(message);
...
function formatMessagesForAnthropic(messages) {
	...
	return {
		messages: (system !== void 0 ? mergedMessages.slice(1) : mergedMessages).map((message) => {
			...
			if ((0, _langchain_core_messages.isAIMessage)(message) && !!message.tool_calls?.length) if (typeof message.content === "string") if (message.content === "") return {
				role,
...
function _makeMessageChunkFromAnthropicEvent(data, fields) {
	if (data.type === "message_start") {
	...
	} else if (data.type === "content_block_start" && data.content_block.type === "tool_use") return new _langchain_core_messages.AIMessageChunk({
		content: fields.coerceContentToString ? "" : [{
			index: data.index,
			...data.content_block,
			input: ""
		...
		if (content !== void 0) return new _langchain_core_messages.AIMessageChunk({
	...
	} else if (data.type === "content_block_delta" && data.delta.type === "input_json_delta") return new _langchain_core_messages.AIMessageChunk({
		content: fields.coerceContentToString ? "" : [{
			index: data.index,
	...
	else if (data.type === "message_stop" && data["amazon-bedrock-invocationMetrics"] !== void 0) return new _langchain_core_messages.AIMessageChunk({
		content: "",
...
function extractToolUseContent(chunk, concatenatedChunks) {
	...
	else newConcatenatedChunks = (0, _langchain_core_utils_stream.concat)(newConcatenatedChunks, chunk);
