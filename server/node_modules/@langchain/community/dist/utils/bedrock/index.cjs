...
function formatMessagesForCohere(messages) {
	const system = messages.filter((system) => system._getType() === "system").filter((m) => typeof m.content === "string").map((m) => m.content).join("\n\n");
	...
	return {
		chatHistory: conversationMessages.slice(0, -1).map((message) => {
			let role;
...
var BedrockLLMInputOutputAdapter = class {
	...
	*/
	...
	static prepareMessagesOutput(provider, response, fields) {
		...
		else if (responseBody.event_type === "text-generation" && typeof responseBody?.text === "string") return new _langchain_core_outputs.ChatGenerationChunk({
			message: new _langchain_core_messages.AIMessageChunk({ content: responseBody.text }),
			text: responseBody.text
		});
		else if (responseBody.event_type === "search-queries-generation") return parseMessageCohere(responseBody);
		else if (responseBody.event_type === "stream-end" && responseBody.response !== void 0 && responseBody["amazon-bedrock-invocationMetrics"] !== void 0) return new _langchain_core_outputs.ChatGenerationChunk({
...
function parseMessage(responseBody, asChunk) {
	...
	if (asChunk) return new _langchain_core_outputs.ChatGenerationChunk({
		message: new _langchain_core_messages.AIMessageChunk({
...
function parseMessageCohere(responseBody, asChunk) {
	...
	if (asChunk) return new _langchain_core_outputs.ChatGenerationChunk({
		message: new _langchain_core_messages.AIMessageChunk({ content: parsedContent }),
		text: parsedContent,
		generationInfo
	});
	else return {
		message: new _langchain_core_messages.AIMessage({ content: parsedContent }),
		text: parsedContent,
		generationInfo
	};
}
