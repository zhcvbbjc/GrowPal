...
var ProxyParamsDuplicator = class ProxyParamsDuplicator {
	...
	*/
	static getFlattenedParams(rpc, filter) {
		...
		filter(new Proxy(rpc, proxiedDuplicator.buildProxyHandler()));
...
function convertObjectFilterToStructuredQuery(objFilter) {
	return new _langchain_core_structured_query.StructuredQuery("", new _langchain_core_structured_query.Operation(_langchain_core_structured_query.Operators.and, Object.entries(objFilter).map(([column, value]) => new _langchain_core_structured_query.Comparison(_langchain_core_structured_query.Comparators.eq, column, value))));
}
//#endregion
