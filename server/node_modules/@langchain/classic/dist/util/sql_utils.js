	...
	if (appDataSource.options.type === "postgres" || appDataSource.options.type === "aurora-postgres") {
		...
		sql = `SELECT 
          ...
          ORDER BY 
            t.table_name,
            c.ordinal_position;`;
		return formatToSqlTable(await appDataSource.query(sql));
	}
...
const generateTableInfoFromTables = async (tables, appDataSource, nbSampleRow, customDescription) => {
	...
	for (const currentTable of tables) {
		...
		let sample = "";
		try {
			sample = formatSqlResponseToSimpleTableString(nbSampleRow ? await appDataSource.query(sqlSelectInfoQuery) : null);
		} catch (error) {
			console.log(error);
		}
