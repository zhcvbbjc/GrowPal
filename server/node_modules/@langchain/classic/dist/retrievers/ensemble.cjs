...
*/
var EnsembleRetriever = class extends _langchain_core_retrievers.BaseRetriever {
	static lc_name() {
	...
	async _rankFusion(query, runManager) {
		...
		return await this._weightedReciprocalRank(retrieverDocs);
	}
	async _weightedReciprocalRank(docList) {
		...
		return Array.from(uniqueDocs).sort((a, b) => rrfScoreDict[b.pageContent] - rrfScoreDict[a.pageContent]);
	}
