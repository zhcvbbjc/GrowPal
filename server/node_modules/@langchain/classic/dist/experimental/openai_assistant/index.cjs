...
var openai_assistant_exports = /* @__PURE__ */ require_runtime.__exportAll({
	OpenAIAssistantRunnable: () => OpenAIAssistantRunnable,
	formatToOpenAIAssistantTool: () => formatToOpenAIAssistantTool
});
var OpenAIAssistantRunnable = class extends _langchain_core_runnables.Runnable {
	lc_namespace = [
		"langchain",
		"experimental",
		"openai_assistant"
	...
	constructor(fields) {
		super(fields);
		this.client = fields.client ?? new _langchain_openai.OpenAIClient(fields?.clientOptions);
	...
	static async createAssistant({ model, name, instructions, tools, client, clientOptions, asAgent, pollIntervalMs, fileIds }) {
		const formattedTools = tools?.map((tool) => {
			if (tool instanceof _langchain_core_tools.StructuredTool) return formatToOpenAIAssistantTool(tool);
			return tool;
		}) ?? [];
		const oaiClient = client ?? new _langchain_openai.OpenAIClient(clientOptions);
	...
	*/
	...
	async _parseStepsInput(input) {
		...
		const toolCalls = (await this._waitForRun(runId, threadId)).required_action?.submit_tool_outputs.tool_calls;
		if (!toolCalls) return input;
		return {
			toolOutputs: toolCalls.flatMap((toolCall) => {
	...
	async _createThreadAndRun(input) {
		...
		return this.client.beta.threads.createAndRun({
			...params,
			thread: input.thread,
			assistant_id: this.assistantId
		});
	...
	async _getResponse(runId, threadId) {
		...
		if (run.status === "completed") {
			const newMessages = (await this.client.beta.threads.messages.list(threadId, { order: "desc" })).data.filter((msg) => msg.run_id === runId);
			if (!this.asAgent) return newMessages;
			const answer = newMessages.flatMap((msg) => msg.content);
			if (answer.every((item) => item.type === "text")) return {
				returnValues: {
					output: answer.map((item) => item.type === "text" && item.text.value).join("\n"),
					runId,
					threadId
				},
				log: "",
