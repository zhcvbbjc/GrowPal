...
var OpenAIAssistantRunnable = class extends Runnable {
	...
	static async createAssistant({ model, name, instructions, tools, client, clientOptions, asAgent, pollIntervalMs, fileIds }) {
		const formattedTools = tools?.map((tool) => {
			if (tool instanceof StructuredTool) return formatToOpenAIAssistantTool(tool);
			return tool;
		}) ?? [];
	...
	*/
	...
	async _waitForRun(runId, threadId) {
		...
		while (inProgress) {
			...
			if (inProgress) await sleep(this.pollIntervalMs);
		}
		return run;
	}
...
//#endregion
export { OpenAIAssistantRunnable, formatToOpenAIAssistantTool, openai_assistant_exports };
