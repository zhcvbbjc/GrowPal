...
var MultiRetrievalQAChain = class MultiRetrievalQAChain extends MultiRouteChain {
	...
	*/
	static fromLLMAndRetrievers(llm, { retrieverNames, retrieverDescriptions, retrievers, retrieverPrompts, defaults, multiRetrievalChainOpts, retrievalQAChainOpts }) {
		...
		const outputParser = new RouterOutputParser(z.object({
		...
		const routerPrompt = new PromptTemplate({
			template: interpolateFString(STRUCTURED_MULTI_RETRIEVAL_ROUTER_TEMPLATE(outputParser.getFormatInstructions({ interpolationDepth: 4 })), { destinations: destinationsStr }),
			inputVariables: ["input"],
			outputParser
		});
		const routerChain = LLMRouterChain.fromLLM(llm, routerPrompt);
		const destinationChains = zipEntries(retrieverNames, retrievers, retrieverPrompts ?? retrievers.map(() => null)).reduce((acc, [name, retriever, prompt]) => {
		...
		else _defaultChain = new ConversationChain({
			llm,
			prompt: new PromptTemplate({
				template: DEFAULT_TEMPLATE.replace("input", "query"),
