...
function computeEvaluationScore(vectors, distanceMetric) {
	return getDistanceCalculationFunction(distanceMetric)(vectors[0], vectors[1]);
}
/**
...
var EmbeddingDistanceEvalChain = class extends StringEvaluator {
	...
	async _call(values, _runManager) {
		...
		const score = computeEvaluationScore(await this.embedding.embedDocuments([prediction, reference]), this.distanceMetric);
...
var PairwiseEmbeddingDistanceEvalChain = class extends PairwiseStringEvaluator {
	...
	async _call(values, _runManager) {
		...
		if (!this.embedding) throw new Error("Embedding is undefined");
		const score = computeEvaluationScore(await this.embedding.embedDocuments([prediction, predictionB]), this.distanceMetric);
		return { [this.outputKey]: score };
	}
};
//#endregion
